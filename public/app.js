...
/* PATCH V20.9.4 AUTO UPPERCASE + MOBILE */
(function(){
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.querySelectorAll('.version,.badge-version').forEach(e=>e.textContent = "V20.9.4");
  });
  setInterval(()=>document.querySelectorAll('.version,.badge-version').forEach(e=>e.textContent = "V20.9.4"),1000);
})();

/* PATCH V20.9.4 FRONT FIX */
(function(){
  function setVersion(){
    document.querySelectorAll(".version,.badge-version").forEach(e=>e.textContent="V20.9.4");
  }
...
/* PATCH V20.9.4 - NOVO CHAMADO ÚNICO + SUGESTÃO DE PRESTADOR MODAL/AUTOMÁTICA */
(function(){
  if(window.__V2094_SUGESTAO__) return;
  window.__V2094_SUGESTAO__ = true;

  function q(s,r=document){return r.querySelector(s)}
  function qa(s,r=document){return [...r.querySelectorAll(s)]}
  function getForm(el){return el.closest('form') || document.querySelector('form')}
  function lojaVal(f){return q('[name="lojaNome"],[name="loja"]',f)?.value || ''}
  function tipoVal(f){return q('[name="tipoServico"],[name="tipoServicoTela"],#tipoServico,#tipoServicoTela',f)?.value || ''}
  function prestInput(f){return q('[name="prestadorNome"],[name="prestador"],#prestadorNome',f)}
  function moeda(v){return Number(v||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'})}

  async function buscarPrestadores(f, abrirModal=false){
    const loja = lojaVal(f), tipo = tipoVal(f), box = q('#sugestoes',f) || q('#sugestoes');
    if(!loja || loja.trim().length < 2){
      if(box) box.innerHTML = 'INFORME A LOJA PARA SUGERIR O PRESTADOR.';
      return;
    }
    if(box) box.innerHTML = 'PESQUISANDO PRESTADORES DISPONÍVEIS...';
    const data = await fetch('/api/prestadores-sugeridos?loja='+encodeURIComponent(loja)+'&tipo='+encodeURIComponent(tipo))
      .then(r=>r.json()).catch(e=>({ok:false,msg:String(e),items:[]}));
    const items = data.items || [];
    const html = items.length ? items.slice(0,10).map(p=>{
      const nome = p.empresa || p.responsavel || p.nome || '';
      const cid = [p.cidade||'', p.uf||p.estado||''].filter(Boolean).join('/');
      const dist = p.distancia ? Number(p.distancia).toFixed(1)+' KM' : 'CIDADE/UF';
      const km = p.valorKm ? ' | R$/KM: '+p.valorKm : '';
      const desl = p.valorDeslocamento ? ' | DESLOC.: '+moeda(p.valorDeslocamento) : '';
      return `<button type="button" class="btn small pick-prestador" data-prestador="${String(nome).replace(/"/g,'&quot;')}">✅ ${nome} - ${cid} | ${dist}${km}${desl}</button>`;
    }).join(' ') : 'NENHUM PRESTADOR ENCONTRADO PARA ESSA LOJA/SERVIÇO.';
    if(box) box.innerHTML = '<b>🎯 SUGESTÃO AUTOMÁTICA POR LOGÍSTICA:</b><br>'+html;

    const modal = q('#modalPrestadores');
    const lista = q('#modalPrestadoresLista');
    if(lista) lista.innerHTML = html;
    if(abrirModal && modal && typeof modal.showModal === 'function') modal.showModal();

    qa('.pick-prestador').forEach(b=>{
      b.onclick = ()=>{
        const inp = prestInput(f);
        if(inp) inp.value = b.dataset.prestador || '';
  ...
  document.addEventListener('click', function(e){
    const btn = e.target.closest('#btnSugestao,#btnSugestaoInline,.v160-sugerir');
    if(!btn) return;
    e.preventDefault();
    buscarPrestadores(getForm(btn), true);
  }, true);

  let timer=null;
  document.addEventListener('change', function(e){
    const el=e.target;
    if(!el || !['lojaNome','loja','tipoServico','tipoServicoTela'].includes(el.name) && !['tipoServico','tipoServicoTela'].includes(el.id)) return;
    const f=getForm(el);
    clearTimeout(timer);
    timer=setTimeout(()=>buscarPrestadores(f,false),350);
  }, true);
})();

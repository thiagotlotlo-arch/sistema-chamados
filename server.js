...
/* PATCH V12.2 FIX USUARIOS PERFIS BACKUP - ROTAS ANTES DA V12.1 */
const telasV122=['INICIO','CHAMADOS','LOJAS','PRESTADORES','PROPRIETARIOS','LEMBRETES','PREVENTIVAS','ORDENS_SERVICO','IMPORTAR','CONFIG','USUARIOS','BACKUP','PONTO_HORAS','RELATORIOS','PAGAMENTOS','TODAS'];
function arrV122(v){return Array.isArray(v)?v:(v?[v]:[])}
function chkV122(a,v){return (a||[]).map(up).includes(up(v))?'checked':''}
function defaultsV122(){const d=db();d.usuarios=d.usuarios||[];d.perfis=d.perfis||[];if(!d.perfis.length)d.perfis=[{id:1,nome:'ADMIN',permissoes:['TODAS']},{id:2,nome:'ANALISTA',permissoes:['INICIO','CHAMADOS','LOJAS','PRESTADORES','PROPRIETARIOS','LEMBRETES','PREVENTIVAS','ORDENS_SERVICO']},{id:3,nome:'CONSULTA',permissoes:['INICIO','CHAMADOS','LOJAS','PRESTADORES']}];let u=d.usuarios.find(x=>up(x.usuario||x.nome)==='OLITECH');if(!u)d.usuarios.push({id:next(d,'proximoUsuario'),nome:'OLITECH',usuario:'OLITECH',senha:'051309',perfil:'ADMIN',ativo:'SIM',analista:'SIM',permissoes:['TODAS']});else{u.usuario='OLITECH';u.senha=u.senha||'051309';u.perfil=u.perfil||'ADMIN';u.ativo=u.ativo||'SIM';u.analista=u.analista||'SIM';u.permissoes=u.permissoes||['TODAS']};save(d)}
try{defaultsV122()}catch(e){console.error('defaults v12.2',e)}
function usuarioFormV122(req,res,u={}){const d=db();const edit=!!u.id;const body=`<div class="bar"><h2>${edit?'EDITAR':'NOVO'} USUÁRIO / ANALISTA</h2><a class="btn secondary" href="/usuarios">Voltar</a></div><form method="post" action="${edit?'/usuarios/'+u.id:'/usuarios'}" class="card form"><div class="grid4"><label>Nome<input name="nome" value="${esc(u.nome||'')}" required></label><label>Usuário/login<input name="usuario" value="${esc(u.usuario||'')}" required></label><label>Senha<input name="senha" value="${esc(u.senha||'')}" required></label><label>Ativo<select name="ativo">${['SIM','NÃO'].map(x=>`<option ${String(u.ativo||'SIM')==x?'selected':''}>${x}</option>`).join('')}</select></label><label>Perfil<select name="perfil">${(d.perfis||[]).map(p=>`<option ${String(u.perfil||'ANALISTA')==String(p.nome)?'selected':''}>${esc(p.nome)}</option>`).join('')}</select></label><label>É analista?<select name="analista">${['SIM','NÃO'].map(x=>`<option ${String(u.analista||'SIM')==x?'selected':''}>${x}</option>`).join('')}</select></label><label>Email<input name="email" value="${esc(u.email||'')}"></label><label>Telefone<input name="telefone" value="${esc(u.telefone||'')}"></label></div><h3>Permissões específicas</h3><div class="perm-grid">${telasV122.map(t=>`<label class="checkline"><input type="checkbox" name="permissoes" value="${t}" ${chkV122(u.permissoes,t)}> ${t.replace('_',' ')}</label>`).join('')}</div><div class="actions"><button>💾 Salvar</button><a class="btn secondary" href="/usuarios">Voltar</a></div></form>`;res.send(layout(req,edit?'Editar usuário':'Novo usuário',body))}
app.get('/usuarios',auth,(req,res)=>{defaultsV122();const d=db();const q=up(req.query.q||'');const lista=(d.usuarios||[]).filter(u=>!q||up(`${u.nome} ${u.usuario} ${u.perfil}`).includes(q));const body=`<div class="bar"><h2>USUÁRIOS / ANALISTAS</h2><a class="btn" href="/usuarios/novo">+ Novo usuário</a><a class="btn secondary" href="/perfis">Perfis/permissões</a><a class="btn secondary" href="/backup">Backup</a></div><form class="card search"><input name="q" value="${esc(req.query.q||'')}" placeholder="Buscar usuário, analista ou perfil"><button>🔎 Buscar</button></form><div class="card"><table><thead><tr><th>Nome</th><th>Usuário</th><th>Perfil</th><th>Analista</th><th>Ativo</th><th>Ações</th></tr></thead><tbody>${lista.map(u=>`<tr><td>${esc(u.nome)}</td><td>${esc(u.usuario)}</td><td>${esc(u.perfil)}</td><td>${esc(u.analista||'')}</td><td>${esc(u.ativo||'SIM')}</td><td><a class="btn small" href="/usuarios/${u.id}/editar">Editar</a></td></tr>`).join('')||'<tr><td colspan="6">Nenhum usuário.</td></tr>'}</tbody></table></div>`;res.send(layout(req,'Usuários',body))});
app.get('/usuarios/novo',auth,(req,res)=>usuarioFormV122(req,res,{}));
app.get('/usuarios/:id/editar',auth,(req,res)=>{const u=(db().usuarios||[]).find(x=>String(x.id)===String(req.params.id));if(!u)return res.redirect('/usuarios');usuarioFormV122(req,res,u)});
app.post('/usuarios',auth,(req,res)=>{const d=db();d.usuarios=d.usuarios||[];d.usuarios.push({id:next(d,'proximoUsuario'),nome:up(req.body.nome),usuario:up(req.body.usuario),senha:String(req.body.senha||''),perfil:up(req.body.perfil||'ANALISTA'),ativo:up(req.body.ativo||'SIM'),analista:up(req.body.analista||'SIM'),email:req.body.email||'',telefone:req.body.telefone||'',permissoes:arrV122(req.body.permissoes).map(up)});save(d);res.redirect('/usuarios')});
app.post('/usuarios/:id',auth,(req,res)=>{const d=db();const u=(d.usuarios||[]).find(x=>String(x.id)===String(req.params.id));if(u){u.nome=up(req.body.nome);u.usuario=up(req.body.usuario);u.senha=String(req.body.senha||'');u.perfil=up(req.body.perfil||'ANALISTA');u.ativo=up(req.body.ativo||'SIM');u.analista=up(req.body.analista||'SIM');u.email=req.body.email||'';u.telefone=req.body.telefone||'';u.permissoes=arrV122(req.body.permissoes).map(up);save(d)}res.redirect('/usuarios')});
function perfilFormV122(req,res,p={}){const edit=!!p.id;const body=`<div class="bar"><h2>${edit?'EDITAR':'NOVO'} PERFIL</h2><a class="btn secondary" href="/perfis">Voltar</a></div><form class="card form" method="post" action="${edit?'/perfis/'+p.id:'/perfis'}"><label>Nome do perfil<input name="nome" value="${esc(p.nome||'')}" required></label><h3>Permissões</h3><div class="perm-grid">${telasV122.map(t=>`<label class="checkline"><input type="checkbox" name="permissoes" value="${t}" ${chkV122(p.permissoes,t)}> ${t.replace('_',' ')}</label>`).join('')}</div><div class="actions"><button>💾 Salvar</button><a class="btn secondary" href="/perfis">Voltar</a></div></form>`;res.send(layout(req,edit?'Editar perfil':'Novo perfil',body))}
app.get('/perfis',auth,(req,res)=>{defaultsV122();const d=db();const body=`<div class="bar"><h2>PERFIS E PERMISSÕES</h2><a class="btn" href="/perfis/novo">+ Novo perfil</a><a class="btn secondary" href="/usuarios">Usuários</a></div><div class="card"><table><thead><tr><th>Perfil</th><th>Permissões</th><th>Ações</th></tr></thead><tbody>${(d.perfis||[]).map(p=>`<tr><td>${esc(p.nome)}</td><td>${esc((p.permissoes||[]).join(', '))}</td><td><a class="btn small" href="/perfis/${p.id}/editar">Editar</a></td></tr>`).join('')}</tbody></table></div>`;res.send(layout(req,'Perfis',body))});
app.get('/perfis/novo',auth,(req,res)=>perfilFormV122(req,res,{}));
app.get('/perfis/:id/editar',auth,(req,res)=>{const p=(db().perfis||[]).find(x=>String(x.id)===String(req.params.id));if(!p)return res.redirect('/perfis');perfilFormV122(req,res,p)});
app.post('/perfis',auth,(req,res)=>{const d=db();d.perfis=d.perfis||[];d.perfis.push({id:next(d,'proximoPerfil'),nome:up(req.body.nome),permissoes:arrV122(req.body.permissoes).map(up)});save(d);res.redirect('/perfis')});
app.post('/perfis/:id',auth,(req,res)=>{const d=db();const p=(d.perfis||[]).find(x=>String(x.id)===String(req.params.id));if(p){p.nome=up(req.body.nome);p.permissoes=arrV122(req.body.permissoes).map(up);save(d)}res.redirect('/perfis')});
app.get('/backup',auth,(req,res)=>res.send(layout(req,'Backup',`<div class="bar"><h2>BACKUP E RESTAURAÇÃO</h2><a class="btn secondary" href="/config">Voltar</a></div><div class="card"><h3>Baixar backup</h3><a class="btn" href="/backup/download">⬇️ Baixar backup</a></div><form class="card" method="post" action="/backup/restaurar" enctype="multipart/form-data" onsubmit="return confirm('Restaurar backup vai substituir os dados atuais. Continuar?')"><h3>Restaurar backup</h3><input type="file" name="backup" accept=".json,application/json" required><div class="actions"><button>♻️ Restaurar</button></div></form>`)));
app.get('/backup/download',auth,(req,res)=>{const nome=`backup-vbchamados-${new Date().toISOString().slice(0,10)}.json`;res.setHeader('Content-Type','application/json; charset=utf-8');res.setHeader('Content-Disposition',`attachment; filename="${nome}"`);res.end(JSON.stringify(db(),null,2))});
app.post('/backup/restaurar',auth,upload.single('backup'),(req,res)=>{try{if(!req.file||!req.file.path)throw new Error('Arquivo não enviado.');const novo=JSON.parse(fs.readFileSync(req.file.path,'utf8'));save(novo);res.send(layout(req,'Backup restaurado',`<div class="card"><h2>BACKUP RESTAURADO</h2><a class="btn" href="/">Início</a></div>`))}catch(e){res.send(layout(req,'Erro backup',`<div class="card"><h2>ERRO AO RESTAURAR</h2><p>${esc(e.message)}</p><a class="btn" href="/backup">Voltar</a></div>`))}});

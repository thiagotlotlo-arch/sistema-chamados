...
/* PATCH V21.0.1 - função de status fechado para evitar CLOSED IS NOT DEFINED */
...
/* PATCH V21.0.1 - moeda BR correta: R$800,00 = 800.00 */
...
/* PATCH V21.0.1 - consolidação das correções pedidas mantendo estrutura V20.8 */
...
/* PATCH V21.0.1 - LOJA: salvar nome editado, anexos/cartão CNPJ persistentes e reutilizar logo */
function v20817_normNomeLoja(req,l){
  const b = req.body || {};
  let nome = String(b.nome || b.nomeLoja || b.lojaNome || l.nome || '').trim().toUpperCase().replace(/\s+/g,' ');
  const cidade = String(b.cidade || l.cidade || '').trim().toUpperCase().replace(/\s+/g,' ');
  const ufRaw = String(b.uf || b.estado || l.uf || l.estado || '').trim().toUpperCase().replace(/\s+/g,' ');
  const ufMap = {'SÃO PAULO':'SP','SAO PAULO':'SP','CEARÁ':'CE','CEARA':'CE','RIO DE JANEIRO':'RJ','MINAS GERAIS':'MG','PARANÁ':'PR','PARANA':'PR','SANTA CATARINA':'SC','RIO GRANDE DO SUL':'RS','BAHIA':'BA'};
  const uf = ufMap[ufRaw] || ufRaw;
  // V21.0.1: em edição manual, respeita exatamente o nome digitado.
  // A montagem automática MEGA VEST CASA + CIDADE + UF fica somente quando a ação foi importação por PDF.
  if(String(b.acao||'') === 'pdf'){
    const cfg = load().config || {};
    const base = String(cfg.nomeBaseFilial || cfg.nomeBaseLoja || 'MEGA VEST CASA').trim().toUpperCase();
    const limpo = nome.replace(/\b(LTDA|ME|EPP|EIRELI|S\/A|SA)\b/g,'').replace(/\s+/g,' ').trim();
    if((limpo === base || limpo === 'MEGA VEST CASA') && cidade && uf) nome = `${base} ${cidade} ${uf}`;
  }
  if(nome){ l.nome = nome; l.nomeLoja = nome; l.lojaNome = nome; }
  if(uf){ l.uf = uf; l.estado = uf; }
  if(!dig(l.cep) && cidade && uf){
    const mapa = {
      'PRAIA GRANDE|SP':'11726000','PIRACICABA|SP':'13400000','FORTALEZA|CE':'60000000',
      'SANTOS|SP':'11000000','SAO PAULO|SP':'01001000','SÃO PAULO|SP':'01001000',
      'GUARUJA|SP':'11410000','GUARUJÁ|SP':'11410000','CAMPINAS|SP':'13010000',
      'SOROCABA|SP':'18010000','RIBEIRAO PRETO|SP':'14010000','RIBEIRÃO PRETO|SP':'14010000'
    };
    l.cep = mapa[`${cidade}|${uf}`] || l.cep || '';
  }
  return l;
}
function v20817_dataFile(file){
  if(!file) return '';
  if(file.dataUrl) return file.dataUrl;
  try{
    const mime=file.mimetype || 'application/octet-stream';
    if(file.buffer) return `data:${mime};base64,${file.buffer.toString('base64')}`;
    if(file.path && fs.existsSync(file.path)) return `data:${mime};base64,${fs.readFileSync(file.path).toString('base64')}`;
  }catch(e){}
  if(file.path) return file.path;
  return '';
}
function v20817_addFileList(target, files){
  const lista = Array.isArray(target) ? target : [];
  for(const f of (files || [])){
    const data = v20817_dataFile(f);
    if(data) lista.push({nome:f.originalname || f.filename || 'ARQUIVO', tipo:f.mimetype || '', dataUrl:data, data:data, criadoEm:new Date().toISOString()});
  }
  return lista;
}
function v20817_aplicaArquivosLoja(req,l,d){
  // Logo reaproveitada: usa logo da empresa ou de outra loja sem duplicar quando selecionado.
  const reap = String(req.body.reutilizarLogo || req.body.logoReutilizar || '').trim();
  if(reap && reap !== 'NÃO' && reap !== 'NAO' && reap !== 'Não alterar'){
    if(reap === 'EMPRESA' && d.config && (d.config.logoEmpresaLocal || d.config.logoAtual || d.config.logoDataUrl)){
      l.logoLocal = d.config.logoEmpresaLocal || d.config.logoAtual || d.config.logoDataUrl;
      l.logoRef = 'EMPRESA';
    } else {
      const outra = (d.lojas||[]).find(x=>String(x.id)===reap || norm(x.nome)===norm(reap));
      if(outra && (outra.logoLocal || outra.logoDataUrl || outra.logoUrl)){
        l.logoLocal = outra.logoLocal || outra.logoDataUrl || outra.logoUrl;
        l.logoRef = outra.id || outra.nome;
      }
    }
  }

  if(req.files?.logoLocal?.[0]){
    const data = v20817_dataFile(req.files.logoLocal[0]);
    if(data){ l.logoLocal = data; l.logoDataUrl = data; l.logoRef = ''; }
  }
  if(req.files?.cartaoCnpj?.[0]){
    const data = v20817_dataFile(req.files.cartaoCnpj[0]);
    if(data){
      l.cartaoCnpj = {nome:req.files.cartaoCnpj[0].originalname || 'CARTAO CNPJ', tipo:req.files.cartaoCnpj[0].mimetype || '', dataUrl:data, data:data, criadoEm:new Date().toISOString()};
    }
  }
  if(req.files?.fotos?.length){
    l.fotos = v20817_addFileList(l.fotos, req.files.fotos);
  }
  return l;
}
...
/* PATCH V21.0.1 - PERSISTÊNCIA SUPABASE APP_STATE SEM ALTERAR FUNÇÕES */
...
const supabasePersist = (SUPABASE_URL && SUPABASE_KEY) ? createClient(SUPABASE_URL, SUPABASE_KEY, {
  ...
  global: { headers: { 'x-application-name': 'vbchamados-v20-8-34' } }
}) : null;
...
async function initPersistentDB(){
  ...
  if(!supabasePersist){
    ...
    console.error('V21.0.1:', lastPersistError);
    return;
  }
  try{
    ...
    if(data && data.data && Object.keys(data.data).length){
      ...
      console.log('V21.0.1 carregado do Supabase app_state:', SUPABASE_STATE_ID);
    }else{
      ...
      if(dbHasRealData(fallback)){
        ...
        console.log('V21.0.1 Supabase estava vazio: enviado backup local com dados para app_state:', SUPABASE_STATE_ID);
      }else{
        const inicial = mergeDB(emptyDB(), {});
        persistentCache = inicial;
        await saveRemoteNow(inicial);
        console.log('V21.0.1 Supabase estava vazio: criado app_state inicial:', SUPABASE_STATE_ID);
      }
      remoteLoaded = true;
    }
  }catch(e){
    supabaseOk = false; remoteLoaded = false; lastSaveOk = false; lastPersistError = e.message || String(e);
    console.error('V21.0.1 ERRO SUPABASE:', lastPersistError);
...
function scheduleRemoteSave(d){
  ...
  try{ fs.writeFileSync(DB_FILE,JSON.stringify(persistentCache,null,2),'utf8'); }catch(e){ console.error('V21.0.1 erro JSON local:', e.message||e); }
  ...
  savingRemote = true;
  setTimeout(async()=>{
    try{ await saveRemoteNow(persistentCache); console.log('V21.0.1 salvo no Supabase app_state:', SUPABASE_STATE_ID); }
    catch(e){ lastSaveOk=false; supabaseOk=false; lastPersistError=e.message||String(e); console.error('V21.0.1 erro ao salvar Supabase:', lastPersistError); }
...
/* PATCH V21.0.1 - GRAVAÇÃO FORTE NO SUPABASE ANTES DA RESPOSTA */
let lastFlushAt = null;
let lastFlushError = '';
async function forcePersistNow(reason='manual'){
  if(!persistentCache) return {ok:false,error:'CACHE VAZIO'};
  try{
    await saveRemoteNow(persistentCache);
    lastFlushAt = new Date().toISOString();
    lastFlushError = '';
    console.log('V21.0.1 persistência confirmada:', reason, lastFlushAt);
    ...
    lastSaveOk = false;
    supabaseOk = false;
    console.error('V21.0.1 FALHA AO CONFIRMAR SUPABASE:', lastFlushError);
    return {ok:false,error:lastFlushError};
  }
}
function installStrongSaveMiddleware(app){
  app.use((req,res,next)=>{
    const mutating = ['POST','PUT','PATCH','DELETE'].includes(String(req.method||'').toUpperCase());
    ...
    const wrap = (methodName)=>{
      ...
      res[methodName] = function(...args){
        ...
        forcePersistNow(`${req.method} ${req.originalUrl||req.url}`).finally(()=>original(...args));
        return res;
      };
    };
    wrap('redirect');
...
/* PATCH V21.0.1 FINAL - PERSISTÊNCIA SEGURA E EDIÇÃO GERAL */
function v20824_dataRoute(req){
  const u=String(req.originalUrl||req.url||'');
  if(u.startsWith('/login')||u.startsWith('/logout')||u.startsWith('/api/save/status')) return false;
  return ['POST','PUT','PATCH','DELETE'].includes(String(req.method||'').toUpperCase());
}
function v20824_saveLocal(d){ try{ fs.writeFileSync(DB_FILE,JSON.stringify(mergeDB(emptyDB(),d||persistentCache||{}),null,2),'utf8'); }catch(e){ console.error('V21.0.1 local save:',e.message||e); } }
function v20824_saveBackground(reason){
  try{
    if(!persistentCache) return;
    v20824_saveLocal(persistentCache);
    if(supabasePersist){
      saveRemoteNow(persistentCache).then(()=>{lastSaveOk=true;lastSaveAt=new Date().toISOString();lastPersistError='';console.log('V21.0.1 salvo no Supabase:',reason);}).catch(e=>{lastSaveOk=false;lastPersistError=e.message||String(e);console.error('V21.0.1 erro Supabase:',lastPersistError);});
    }
  }catch(e){ console.error('V21.0.1 background:',e.message||e); }
}
function v20824_installSafeSave(app){ app.use((req,res,next)=>{ if(v20824_dataRoute(req)) res.on('finish',()=>v20824_saveBackground(`${req.method} ${req.originalUrl||req.url}`)); next(); }); }
function v20824_mergeBody(obj,body){
  body=body||{}; obj=obj||{};
  for(const [k,v] of Object.entries(body)){
    if(['acao','permissoes','servicos','chamados'].includes(k)) continue;
    if(v===undefined) continue;
    obj[k]=Array.isArray(v)?v:(typeof v==='string'?v.trim().toUpperCase():v);
  }
  return obj;
}
function v20824_fixLoja(req,l,d){
  if(!l) return l; const b=req.body||{};
  let nome=String(b.nome||b.nomeLoja||b.lojaNome||l.nome||'').trim().toUpperCase().replace(/\s+/g,' ');
  const cidade=String(b.cidade||l.cidade||'').trim().toUpperCase().replace(/\s+/g,' ');
  let uf=String(b.uf||b.estado||l.uf||l.estado||'').trim().toUpperCase().replace(/\s+/g,' ');
  const ufMap={'SÃO PAULO':'SP','SAO PAULO':'SP','CEARÁ':'CE','CEARA':'CE','RIO DE JANEIRO':'RJ','MINAS GERAIS':'MG','PARANÁ':'PR','PARANA':'PR','SANTA CATARINA':'SC','RIO GRANDE DO SUL':'RS','BAHIA':'BA'}; uf=ufMap[uf]||uf;
  if(String(b.acao||'')==='pdf'){
    const base=String((d?.config?.nomeBaseFilial||d?.config?.nomeBaseLoja||d?.config?.nomeBase||'MEGA VEST CASA')).trim().toUpperCase();
    const limpo=nome.replace(/\b(LTDA|ME|EPP|EIRELI|S\/A|SA)\b/g,'').replace(/\s+/g,' ').trim();
    if((limpo===base||limpo==='MEGA VEST CASA') && cidade && uf) nome=`${base} ${cidade} ${uf}`;
  }
  if(nome){l.nome=nome;l.nomeLoja=nome;l.lojaNome=nome;}
...
function v20824_fileData(f){ if(!f)return null; const o=fileObj(f)||{}; try{ if(f.path && !o.dataUrl) o.dataUrl='data:'+(f.mimetype||'application/octet-stream')+';base64,'+fs.readFileSync(f.path).toString('base64'); }catch(e){} return o; }
function v20824_statusOS(o){ return norm(o.status||o.statusOs||'ABERTA'); }
function v20824_osFinal(o){ return ['FINALIZADO','FINALIZADA','CANCELADO','CANCELADA','FECHADO','FECHADA'].includes(v20824_statusOS(o)); }
function v20824_osChamados(d,o){ return os83ChamadosDaOS ? os83ChamadosDaOS(d,o) : []; }
...
function save(d){ persistentCache=mergeDB(emptyDB(), d); v20824_saveLocal(persistentCache); scheduleRemoteSave(persistentCache); return true } function next(d,k){d.config.next[k]=Number(d.config.next[k]||1);return d.config.next[k]++}
...
function page(req,title,body){
  const d=load(),c=d.config,logo=appLogo(d);
  return `<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(c.nomeSistema)} - ${esc(title)}</title><link rel="stylesheet" href="/public/style.css"><style>
@media (max-width:768px){
  body{font-size:15px;overflow-x:hidden} header{padding:14px 12px!important} .brand{gap:10px!important;align-items:center!important} .brand img{max-width:76px!important;max-height:54px!important} .brand h1{font-size:24px!important;line-height:1.1!important} .brand p{font-size:13px!important}
  nav,.menu,.nav{display:grid!important;grid-template-columns:1fr 1fr!important;gap:10px!important;width:100%!important} header a.btn,header button,.menu a,.nav a{width:100%!important;min-height:52px!important;padding:14px 10px!important;margin:0!important;font-size:15px!important;border-radius:14px!important;white-space:normal!important;text-align:center!important;display:flex!important;align-items:center!important;justify-content:center!important}
  main{padding:14px!important} .card{padding:14px!important;border-radius:18px!important}.bar{display:flex!important;gap:10px!important;align-items:center!important;flex-wrap:wrap!important}.bar h2{font-size:22px!important;width:100%!important}.bar .btn{flex:1!important;text-align:center!important}
  .grid4,.grid3,.grid2,.loja-grid,.horarios-grid{display:grid!important;grid-template-columns:1fr!important;gap:12px!important} label{font-size:14px!important} input,select,textarea{width:100%!important;min-height:46px!important;font-size:16px!important;box-sizing:border-box!important} textarea{min-height:95px!important}.actions{display:grid!important;grid-template-columns:1fr!important;gap:10px!important}.actions button,.actions .btn{width:100%!important;min-height:48px!important}
  table{font-size:13px!important} .table-wrap,table{display:block!important;overflow-x:auto!important;max-width:100%!important}.version{position:fixed!important;right:10px!important;bottom:10px!important;z-index:9999!important;font-size:13px!important}.inline-form{display:inline-block!important;margin-left:4px!important}
}
</style></head><body class="theme-${esc(norm(c.tema).toLowerCase())}">${user(req)?`<header><div class="brand">${logo?`<img src="${esc(logo)}" onerror="this.style.display='none'">`:`<div class="logo-fallback">VB</div>`}<div><h1>${esc(c.nomeSistema)}</h1><p>${esc(c.subtitulo)}</p></div></div>${menu(req)}</header>`:''}<main>${body}</main><div class="version">V21.0.2</div><script src="/public/app.js"></script><script>
(function(){
  function ready(fn){ if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',fn);}else{fn();} }
  ready(function(){
    var vers='V21.0.1';
    var vs=document.querySelectorAll('.version,.badge-version');
    for(var i=0;i<vs.length;i++){vs[i].textContent=vers;}
    var path=location.pathname||'';
    var isNew=path.indexOf('/novo')>-1 || path.indexOf('/nova')>-1;
    if(isNew){
      var els=document.querySelectorAll('form input,form select,form textarea');
      for(var j=0;j<els.length;j++){els[j].disabled=false;els[j].readOnly=false;}
    }
    var inputs=document.querySelectorAll('input');
    for(var k=0;k<inputs.length;k++){
      var inp=inputs[k];
      if(inp.getAttribute('data-auto')) continue;
      var txt=((inp.name||'')+' '+(inp.placeholder||'')).toUpperCase();
      if(txt.indexOf('LOJA')>-1) inp.setAttribute('data-auto','lojas');
      else if(txt.indexOf('PRESTADOR')>-1) inp.setAttribute('data-auto','prestadores');
      else if(txt.indexOf('PROPRIET')>-1) inp.setAttribute('data-auto','proprietarios');
      else if(txt.indexOf('ANALISTA')>-1 || txt.indexOf('USUARIO')>-1) inp.setAttribute('data-auto','analistas');
      else if(txt.indexOf('SERVI')>-1) inp.setAttribute('data-auto','servicos');
      else if(txt.indexOf('CHAMADO')>-1) inp.setAttribute('data-auto','chamados');
    }
    var params=new URLSearchParams(location.search);
    if(params.get('valor')==='0'){
      var hs=document.querySelectorAll('h3,h4');
      for(var h=0;h<hs.length;h++){
        if((hs[h].textContent||'').indexOf('VALOR')>-1){
          hs[h].style.display='none';
          if(hs[h].nextElementSibling) hs[h].nextElementSibling.style.display='none';
        }
      }
    }
  });
...
function tabela(headers,rows,empty='Nenhum registro encontrado'){const body=Array.isArray(rows)?(rows.length?rows.join(''):''):(rows?String(rows):'');return `<table><thead><tr>${headers.map(h=>`<th>${esc(h)}</th>`).join('')}</tr></thead><tbody>${body||`<tr><td colspan="${headers.length}">${esc(empty)}</td></tr>`}</tbody></table>`}
...
async function parsePdf(file){
  ...
  function afterLogradouro(labels){
    const start=lines.findIndex(l=>/^LOGRADOURO$|^ENDERECO$|^ENDEREÇO$/.test(l));
    const end=lines.findIndex((l,i)=>i>start && /^CEP$/.test(l));
    const lim=end>start?end:Math.min(lines.length,start+20);
    for(const lab of labels){
      for(let i=Math.max(0,start); i<lim; i++){
        if(lines[i]===lab || lines[i].startsWith(lab+' ')){
          const same=lines[i].replace(lab,'').replace(/^[:\-\s]+/,'').trim();
          if(same) return same;
          for(let k=i+1;k<lim;k++){
            const v=lines[k];
            if(v && !/^(LOGRADOURO|ENDERECO|ENDEREÇO|NUMERO|NÚMERO|COMPLEMENTO|BAIRRO|DISTRITO)$/.test(v)) return v;
          }
        }
      }
    }
  ...
  let numero=afterLogradouro(['NÚMERO','NUMERO']);
  let complemento=afterLogradouro(['COMPLEMENTO']);
  ...
  numero=norm(numero).replace(/\s+/g,' ').trim();
  complemento=norm(complemento).replace(/\s+/g,' ').trim();
  return {nome:limpaEmpresa(nome),razao:limpaEmpresa(razao),fantasia:limpaEmpresa(fantasia),cnpj:dig(cnpj),cep:dig(cep),uf,telefone:dig(tel),cidade,endereco,numero,numeroEndereco:numero,complemento};
}
...
app.get('/login',(req,res)=>res.send(`<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Login</title><link rel="stylesheet" href="/public/style.css"><style>
@media (max-width:768px){
  body{font-size:15px;overflow-x:hidden} header{padding:14px 12px!important} .brand{gap:10px!important;align-items:center!important} .brand img{max-width:76px!important;max-height:54px!important} .brand h1{font-size:24px!important;line-height:1.1!important} .brand p{font-size:13px!important}
  nav,.menu,.nav{display:grid!important;grid-template-columns:1fr 1fr!important;gap:10px!important;width:100%!important} header a.btn,header button,.menu a,.nav a{width:100%!important;min-height:52px!important;padding:14px 10px!important;margin:0!important;font-size:15px!important;border-radius:14px!important;white-space:normal!important;text-align:center!important;display:flex!important;align-items:center!important;justify-content:center!important}
  main{padding:14px!important} .card{padding:14px!important;border-radius:18px!important}.bar{display:flex!important;gap:10px!important;align-items:center!important;flex-wrap:wrap!important}.bar h2{font-size:22px!important;width:100%!important}.bar .btn{flex:1!important;text-align:center!important}
...
/* PATCH V21.0.1 - HOME MOBILE RÁPIDA: GRID SÓ APÓS BUSCA/MOSTRAR TODOS */
...
/* PATCH V21.0.1 - preserva nome da loja exatamente como exibido no formulário ao SALVAR */
...
/* PATCH V21.0.1 - nome da loja importada por PDF preservado no salvar */
...
function lojaForm(req,l={}){
  const d=load(),edit=!!l.id;
  const cartao = l.cartaoCnpj ? `<a class="btn small" href="/lojas/${l.id}/cartao-cnpj" target="_blank">📄 ABRIR CARTÃO CNPJ</a><form class="inline-form" method="post" action="/lojas/${l.id}/editar"><input type="hidden" name="removerCartaoCnpj" value="1"><button class="small danger" title="Remover cartão CNPJ">❌</button></form>` : '';
  const fotos = arr(l.fotos).map((f,i)=>`<a class="btn small" href="/lojas/${l.id}/foto/${i}" target="_blank">🖼️ FOTO ${i+1}</a><form class="inline-form" method="post" action="/lojas/${l.id}/editar"><input type="hidden" name="removerFoto" value="${i}"><button class="small danger" title="Remover foto">❌</button></form>`).join(' ');
  return page(req,edit?'Editar loja':'Nova loja',`<div class="bar"><h2>${edit?'✏️ Editar':'➕ Nova'} loja</h2><a class="btn secondary" href="/lojas">Voltar</a></div>
  <form class="card form" method="post" enctype="multipart/form-data">
    <details open><summary>📄 Preencher por PDF/cartão CNPJ</summary><input type="file" name="pdf" accept=".pdf"><button name="acao" value="pdf" formnovalidate>🔎 Pesquisar PDF</button><p class="hint">Ao importar PDF de filial, aplica a regra da Config: MEGA VEST CASA + CIDADE + UF e preenche endereço, número e complemento.</p></details>
    <div class="grid4 loja-grid">
      <label>Código/Filial<input name="codigo" value="${esc(l.codigo||l.id||'')}"></label>
      <label>Tipo código<select name="tipoCodigo"><option>${esc(l.tipoCodigo||'SOMENTE NÚMERO')}</option><option>SOMENTE NÚMERO</option><option>NOME + NÚMERO</option></select></label>
      <label>Nome loja<input name="nome" value="${esc(l.nome)}" required></label>
      <label>Responsável loja<input name="responsavel" value="${esc(l.responsavel)}"></label>
      <label>CNPJ<input name="cnpj" value="${esc(l.cnpj)}" data-mask="cnpj"></label>
      <label>I.E.<input name="ie" value="${esc(l.ie)}"></label>
      <label>Telefone<input name="telefone" value="${esc(l.telefone)}" data-mask="telefone"></label>
      <label>WhatsApp responsável<input name="whatsappResponsavel" value="${esc(l.whatsappResponsavel||'')}" data-mask="telefone"></label>
      <label>CEP<input name="cep" value="${esc(l.cep)}" data-mask="cep"></label>
      <label>Estado/UF<input name="uf" value="${esc(l.uf||l.estado)}"></label>
      <label>Cidade<input name="cidade" value="${esc(l.cidade)}"></label>
      <label>Endereço<input name="endereco" value="${esc(l.endereco)}"></label>
      <label>Número<input name="numero" value="${esc(l.numero||l.numeroEndereco||'')}"></label>
      <label>Complemento<input name="complemento" value="${esc(l.complemento||'')}"></label>
      <label>Latitude<input name="latitude" value="${esc(l.latitude)}"></label>
      <label>Longitude<input name="longitude" value="${esc(l.longitude)}"></label>
      <label>Analista responsável<input name="analista" value="${esc(l.analista)}" data-auto="analistas"></label>
      <label>Proprietário<input name="proprietario" value="${esc(l.proprietario)}" data-auto="proprietarios"></label>
      <label>Logo URL<input name="logoUrl" value="${esc(l.logoUrl)}"></label>
      <label>Logo local<input type="file" name="logoLocal" accept="image/*"></label>
      <label>Cartão CNPJ<input type="file" name="cartaoCnpj" accept=".pdf,image/*"></label>
      <label>Fotos<input type="file" name="fotos" multiple accept="image/*"></label>
      <div class="span-full hint"><b>ARQUIVOS SALVOS DA LOJA:</b> ${cartao} ${fotos}</div>
    </div>
    <details open><summary>⏰ Horário funcionamento</summary>
      <div class="grid4 horarios-grid">
        <label>Seg-sex abre<input name="horaSegSexAbre" data-mask="hora" value="${esc(l.horaSegSexAbre)}" placeholder="HH:MM"></label>
        <label>Seg-sex fecha<input name="horaSegSexFecha" data-mask="hora" value="${esc(l.horaSegSexFecha)}" placeholder="HH:MM"></label>
        <label>Sábado abre<input name="horaSabAbre" data-mask="hora" value="${esc(l.horaSabAbre)}" placeholder="HH:MM"></label>
        <label>Sábado fecha<input name="horaSabFecha" data-mask="hora" value="${esc(l.horaSabFecha)}" placeholder="HH:MM"></label>
        <label>Domingo abre<input name="domingoAbre" data-mask="hora" value="${esc(l.domingoAbre||l.horaDomAbre||l.horaDomFeriadoAbre||'')}" placeholder="HH:MM ou FECHADO"></label>
        <label>Domingo fecha<input name="domingoFecha" data-mask="hora" value="${esc(l.domingoFecha||l.horaDomFecha||l.horaDomFeriadoFecha||'')}" placeholder="HH:MM ou FECHADO"></label>
        <label>Feriado abre<input name="feriadoAbre" data-mask="hora" value="${esc(l.feriadoAbre||l.horaFeriadoAbre||l.horaDomFeriadoAbre||'')}" placeholder="HH:MM ou FECHADO"></label>
        <label>Feriado fecha<input name="feriadoFecha" data-mask="hora" value="${esc(l.feriadoFecha||l.horaFeriadoFecha||l.horaDomFeriadoFecha||'')}" placeholder="HH:MM ou FECHADO"></label>
      </div>
      <textarea name="horario" placeholder="Observações">${esc(l.horario)}</textarea>
    </details>
    <div class="actions"><button>💾 Salvar</button><button type="button" data-cep>🔎 Buscar CEP</button><button type="button" data-cnpj>🔎 Buscar CNPJ</button><button type="button" data-geo>📍 Gerar localização</button></div>
  </form>`)
}
app.get('/lojas',auth,need('LOJAS'),(req,res)=>{const d=load(),q=norm(req.query.q||''),show=req.query.mostrar||q;const lista=show?d.lojas.filter(l=>!q||norm([l.nome,l.codigo,l.cidade,l.uf,l.cnpj,l.cep,l.telefone].join(' ')).includes(q)):[];res.send(page(req,'Lojas',`<div class="bar"><h2>🏬 Lojas</h2><a class="btn" href="/lojas/nova">➕ Nova loja</a></div>${busca('/lojas','Buscar loja, cidade, CNPJ, CEP...')}<div class="card">${tabela(['Código','Loja','Cidade/UF','Analista','Telefone','Ações'],lista.map(l=>`<tr><td>${esc(l.codigo||l.id)}</td><td>${esc(l.nome)}</td><td>${esc(l.cidade)}/${esc(l.uf)}</td><td>${esc(l.analista)}</td><td>${esc(l.telefone)}</td><td><a class="btn small" href="/lojas/${l.id}/editar">✏️ Editar</a><form class="inline-form" method="post" action="/lojas/${l.id}/excluir"><button class="small danger">🗑️ Excluir</button></form></td></tr>`),'Use a busca ou Mostrar todos.')}</div>`))});app.get('/lojas/nova',auth,need('LOJAS'),(req,res)=>res.send(lojaForm(req)));app.get('/lojas/:id/editar',auth,need('LOJAS'),(req,res)=>res.send(lojaForm(req,load().lojas.find(l=>String(l.id)===String(req.params.id))||{})));app.post(['/lojas/nova','/lojas/:id/editar'],auth,need('LOJAS'),upload.fields([{name:'pdf',maxCount:1},{name:'logoLocal',maxCount:1},{name:'cartaoCnpj',maxCount:1},{name:'fotos',maxCount:20}]),async(req,res)=>{try{const d=load();let l=req.params.id?d.lojas.find(x=>String(x.id)===String(req.params.id)):null;if(!l){l={id:next(d,'loja')};d.lojas.push(l)}if(req.body.removerCartaoCnpj){delete l.cartaoCnpj; save(d); return res.redirect(`/lojas/${l.id}/editar`)}if(req.body.removerFoto!==undefined && req.body.removerFoto!==''){const idx=Number(req.body.removerFoto); l.fotos=arr(l.fotos).filter((_,i)=>i!==idx); save(d); return res.redirect(`/lojas/${l.id}/editar`)}if(req.body.acao==='pdf'&&req.files?.pdf?.[0]){const p=await parsePdf(req.files.pdf[0].path);Object.assign(req.body,{...req.body,...p});if(d.config.regraNomeFilial!=='ORIGINAL'&&p.nome&&p.cidade){req.body.nome=mesclaFilial(p.nome,p.cidade,p.uf||req.body.uf);req.body.nomeSugestaoFilial=req.body.nome}}const nomeDigitado=String(req.body.nome||'').trim();Object.assign(l,{codigo:req.body.codigo||l.codigo||l.id,tipoCodigo:req.body.tipoCodigo,nome:nomeDigitado?norm(nomeDigitado):(l.nome||''),responsavel:norm(req.body.responsavel),cnpj:dig(req.body.cnpj),ie:norm(req.body.ie),telefone:dig(req.body.telefone),whatsappResponsavel:dig(req.body.whatsappResponsavel),cep:dig(req.body.cep),uf:norm(req.body.uf||req.body.estado),estado:norm(req.body.uf||req.body.estado),cidade:norm(req.body.cidade),endereco:norm(req.body.endereco),numero:norm(req.body.numero||req.body.numeroEndereco),numeroEndereco:norm(req.body.numero||req.body.numeroEndereco),complemento:norm(req.body.complemento),latitude:req.body.latitude||'',longitude:req.body.longitude||'',analista:norm(req.body.analista),proprietario:norm(req.body.proprietario),feriado:norm(req.body.feriado||'FECHADO'),domFeriadoStatus:norm(req.body.domFeriadoStatus||''),horaSegSexAbre:req.body.horaSegSexAbre||'',horaSegSexFecha:req.body.horaSegSexFecha||'',horaSabAbre:req.body.horaSabAbre||'',horaSabFecha:req.body.horaSabFecha||'',horaDomAbre:req.body.horaDomAbre||req.body.horaDomFeriadoAbre||'',horaDomFecha:req.body.horaDomFecha||req.body.horaDomFeriadoFecha||'',domingoAbre:req.body.horaDomAbre||req.body.domingoAbre||req.body.horaDomFeriadoAbre||'',domingoFecha:req.body.domingoFecha||req.body.horaDomFecha||req.body.horaDomFeriadoFecha||'',feriadoAbre:req.body.feriadoAbre||req.body.horaFeriadoAbre||req.body.horaDomFeriadoAbre||'',feriadoFecha:req.body.feriadoFecha||req.body.horaFeriadoFecha||req.body.horaDomFeriadoFecha||'',horaFeriadoAbre:req.body.feriadoAbre||req.body.horaFeriadoAbre||'',horaFeriadoFecha:req.body.feriadoFecha||req.body.horaFeriadoFecha||'',horaDomFeriadoAbre:req.body.horaDomAbre||req.body.domingoAbre||req.body.horaDomFeriadoAbre||'',horaDomFeriadoFecha:req.body.horaDomFecha||req.body.domingoFecha||req.body.horaDomFeriadoFecha||'',domingoFeriadoStatus:norm(req.body.domFeriadoStatus||req.body.domingoFeriadoStatus||''),logoUrl:req.body.logoUrl||'',horario:req.body.horario||''});v20817_normNomeLoja(req,l);if(req.files?.logoLocal?.[0])l.logoLocal=fileObj(req.files.logoLocal[0]);if(req.files?.cartaoCnpj?.[0])l.cartaoCnpj=fileObj(req.files.cartaoCnpj[0]);v20824_mergeBody(l,req.body);v20817_aplicaArquivosLoja(req,l,d);v20824_fixLoja(req,l,d);if(String(req.body.acao||'')!=='pdf' && nomeDigitado){l.nome=norm(nomeDigitado);l.nomeLoja=l.nome;l.lojaNome=l.nome;}save(d);res.redirect(`/lojas/${l.id}/editar`)}catch(e){res.status(500).send(errorPage(req,e))}});app.post('/lojas/:id/excluir',auth,need('LOJAS'),(req,res)=>{const d=load();d.lojas=d.lojas.filter(x=>String(x.id)!==String(req.params.id));save(d);res.redirect('/lojas')});

function prestadorForm(req,p={}){const d=load();return page(req,p.id?'Editar prestador':'Novo prestador',`<div class="bar"><h2>🧰 ${p.id?'Editar':'Novo'} prestador</h2><a class="btn secondary" href="/prestadores">Voltar</a></div><form class="card form" method="post" enctype="multipart/form-data"><details open><summary>📄 Preencher por PDF/cartão CNPJ</summary><input type="file" name="pdf" accept=".pdf"><button name="acao" value="pdf" formnovalidate>🔎 Pesquisar PDF</button><p class="hint">Ao importar PDF de filial, aplica a regra da Config: MEGA VEST CASA + CIDADE + UF.</p></details><div class="grid4"><label>Empresa<input name="empresa" value="${esc(p.empresa)}"></label><label>Nome responsável<input name="responsavel" value="${esc(p.responsavel)}"></label><label>Telefone<input name="telefone" value="${esc(p.telefone)}" data-mask="telefone"></label><label>Email<input name="email" value="${esc(p.email)}"></label><label>CNPJ<input name="cnpj" value="${esc(p.cnpj)}" data-mask="cnpj"></label><label>CPF<input name="cpf" value="${esc(p.cpf)}" data-mask="cpf"></label><label>CPF/CNH<input name="cpfCnh" value="${esc(p.cpfCnh)}"></label><label>CEP<input name="cep" value="${esc(p.cep)}" data-mask="cep"></label><label>Estado/UF<input name="uf" value="${esc(p.uf||p.estado)}"></label><label>Cidade<input name="cidade" value="${esc(p.cidade)}"></label><label>Endereço<input name="endereco" value="${esc(p.endereco)}"></label><label>Ativo<select name="ativo"><option ${norm(p.ativo||'SIM')==='SIM'?'selected':''}>SIM</option><option ${norm(p.ativo)==='NÃO'?'selected':''}>NÃO</option></select></label><label>Raio KM<input name="raioKm" value="${esc(p.raioKm)}"></label><label>Valor por KM<input name="valorKm" value="${esc(p.valorKm)}"></label><label>Latitude<input name="latitude" value="${esc(p.latitude)}"></label><label>Longitude<input name="longitude" value="${esc(p.longitude)}"></label><label>Tipo pagamento<select name="tipoPagamento"><option>${esc(p.tipoPagamento||'PIX')}</option><option>PIX</option><option>CONTA</option><option>DINHEIRO</option></select></label><label>Dados pagamento<input name="dadosPagamento" value="${esc(p.dadosPagamento)}"></label><label>Logo URL<input name="logoUrl" value="${esc(p.logoUrl)}"></label><label>Logo local<input type="file" name="logoLocal"></label><label>Cartão CNPJ<input type="file" name="cartaoCnpj"></label><label>Fotos<input type="file" name="fotos" multiple></label><div class="span-full hint"><b>ARQUIVOS SALVOS DO PROPRIETÁRIO:</b> ${p.cartaoCnpj?`<span class="btn small">📄 CARTÃO CNPJ SALVO</span>`:""} ${(p.fotos||[]).map((f,i)=>`<span class="btn small">🖼️ FOTO ${i+1}</span>`).join(" ")}</div></div><h3>Serviços realizados</h3><details open><summary>Serviços selecionados</summary><div class="perm-grid" id="servicosGrid">${d.tiposServico.map(s=>`<label class="checkline"><input type="checkbox" name="servicos" value="${esc(s)}" ${arr(p.servicos).map(norm).includes(norm(s))?'checked':''}> ${esc(s)}</label>`).join('')}</div></details><div class="actions"><button>💾 Salvar</button><button type="button" data-cep>🔎 Buscar CEP</button><button type="button" data-cnpj>🔎 Buscar CNPJ</button><button type="button" data-geo>📍 Gerar localização</button></div></form>`)}
app.get('/prestadores',auth,need('PRESTADORES'),(req,res)=>{const d=load(),q=norm(req.query.q||''),show=req.query.mostrar||q;const lista=show?d.prestadores.filter(p=>!q||norm([p.empresa,p.responsavel,p.cidade,p.uf,p.cnpj,p.cpf,p.telefone,arr(p.servicos).join(' ')].join(' ')).includes(q)):[];res.send(page(req,'Prestadores',`<div class="bar"><h2>🧰 Prestadores</h2><a class="btn" href="/prestadores/novo">➕ Novo</a></div>${busca('/prestadores','Buscar prestador, CNPJ, cidade, serviço...')}<div class="card">${tabela(['Empresa','Responsável','Cidade/UF','Telefone','Serviços','Ações'],lista.map(p=>`<tr><td>${esc(p.empresa)}</td><td>${esc(p.responsavel)}</td><td>${esc(p.cidade)}/${esc(p.uf)}</td><td>${esc(p.telefone)}</td><td>${esc(arr(p.servicos).join(', '))}</td><td><a class="btn small" href="/prestadores/${p.id}/editar">✏️ Editar</a><form class="inline-form" method="post" action="/prestadores/${p.id}/excluir"><button class="small danger">🗑️ Excluir</button></form></td></tr>`),'Use a busca ou Mostrar todos.')}</div>`))});app.get('/prestadores/novo',auth,need('PRESTADORES'),(req,res)=>res.send(prestadorForm(req)));app.get('/prestadores/:id/editar',auth,need('PRESTADORES'),(req,res)=>res.send(prestadorForm(req,load().prestadores.find(p=>String(p.id)===String(req.params.id))||{})));app.post(['/prestadores/novo','/prestadores/:id/editar'],auth,need('PRESTADORES'),upload.fields([{name:'pdf',maxCount:1},{name:'logoLocal',maxCount:1},{name:'cartaoCnpj',maxCount:1},{name:'fotos',maxCount:20}]),async(req,res)=>{try{const d=load();let p=req.params.id?d.prestadores.find(x=>String(x.id)===String(req.params.id)):null;if(!p){p={id:next(d,'prestador')};d.prestadores.push(p)}if(req.body.acao==='pdf'&&req.files?.pdf?.[0])Object.assign(req.body,{...req.body,...await parsePdf(req.files.pdf[0].path)});Object.assign(p,{empresa:norm(req.body.empresa||req.body.nome||req.body.razao||req.body.fantasia),responsavel:norm(req.body.responsavel||req.body.nome||req.body.razao||req.body.fantasia),telefone:dig(req.body.telefone),email:req.body.email||'',cnpj:dig(req.body.cnpj),cpf:dig(req.body.cpf),cpfCnh:req.body.cpfCnh||'',cep:dig(req.body.cep),uf:norm(req.body.uf),estado:norm(req.body.uf),cidade:norm(req.body.cidade),endereco:norm(req.body.endereco),ativo:norm(req.body.ativo||'SIM'),raioKm:req.body.raioKm||'',valorKm:req.body.valorKm||'',latitude:req.body.latitude||'',longitude:req.body.longitude||'',tipoPagamento:req.body.tipoPagamento||'PIX',dadosPagamento:req.body.dadosPagamento||'',servicos:arr(req.body.servicos).map(norm),logoUrl:req.body.logoUrl||''});if(req.files?.logoLocal?.[0])p.logoLocal=fileObj(req.files.logoLocal[0]);if(req.files?.cartaoCnpj?.[0])p.cartaoCnpj=fileObj(req.files.cartaoCnpj[0]);v20824_mergeBody(p,req.body);if(req.files?.cartaoCnpj?.[0])p.cartaoCnpj=v20824_fileData(req.files.cartaoCnpj[0]);p.fotos=[...arr(p.fotos),...manyFiles(req,'fotos')];save(d);res.redirect(`/prestadores/${p.id}/editar`)}catch(e){res.status(500).send(errorPage(req,e))}});app.post('/prestadores/:id/excluir',auth,need('PRESTADORES'),(req,res)=>{const d=load();d.prestadores=d.prestadores.filter(x=>String(x.id)!==String(req.params.id));save(d);res.redirect('/prestadores')});
...
/* ================= PATCH V21.0.1 - OS AGRUPADA + ASSINATURA + WHATSAPP LOJA ================= */
...
function os83GroupKey(c){ return `${String(c.lojaId||'')||v20830_tipoLoja(c.lojaNome||c.loja||'')+'|'+v20830_limpo(c.lojaNome||c.loja||'SEM LOJA')}|||${String(c.prestadorId||'')||v20830_limpo(c.prestadorNome||c.prestador||'SEM PRESTADOR')}`; }
function os83Loja(d,c,os={}){ return (d.lojas||[]).find(l=>String(l.id)===String(c.lojaId||os.lojaId||'')) || (d.lojas||[]).find(l=>v20830_limpo(l.nome)===v20830_limpo(c.lojaNome||os.lojaNome||c.loja||'')) || findLoja(d,c.lojaNome||os.lojaNome||c.loja||'') || {}; }
function os83Prestador(d,c,os={}){ return (d.prestadores||[]).find(p=>String(p.id)===String(c.prestadorId||os.prestadorId||'')) || (d.prestadores||[]).find(p=>v20830_limpo(p.empresa||p.nome||p.responsavel)===v20830_limpo(c.prestadorNome||os.prestadorNome||c.prestador||'')) || findPrestador(d,c.prestadorNome||os.prestadorNome||c.prestador||'') || {}; }
...
function os83WhatsappLink(tel,msg){ tel=dig(tel); if(!tel)return '#'; if(!tel.startsWith('55'))tel='55'+tel; return `https://wa.me/${tel}`; }
...
/* PATCH V21.0.1 - anexos prestadores */
app.get('/prestadores/:id/cartao-cnpj',auth,need('PRESTADORES'),(req,res)=>{const p=(load().prestadores||[]).find(x=>String(x.id)===String(req.params.id));const f=p?.cartaoCnpj;const data=f?.dataUrl||f?.data||'';if(!data)return res.status(404).send('Arquivo não encontrado');if(String(data).startsWith('data:')){const m=String(data).match(/^data:([^;]+);base64,(.*)$/);if(m){res.setHeader('Content-Type',m[1]);return res.end(Buffer.from(m[2],'base64'));}}res.redirect(data)});
app.get('/prestadores/:id/foto/:idx',auth,need('PRESTADORES'),(req,res)=>{const p=(load().prestadores||[]).find(x=>String(x.id)===String(req.params.id));const f=(p?.fotos||[])[Number(req.params.idx)];const data=f?.dataUrl||f?.data||'';if(!data)return res.status(404).send('Arquivo não encontrado');if(String(data).startsWith('data:')){const m=String(data).match(/^data:([^;]+);base64,(.*)$/);if(m){res.setHeader('Content-Type',m[1]);return res.end(Buffer.from(m[2],'base64'));}}res.redirect(data)});


app.get('/os',auth,need('ORDENS_SERVICO'),(req,res)=>{
  const d=os83State(); const q=norm(req.query.q||''); const status=norm(req.query.status||'ABERTAS');
  let lista=(d.os||[]).slice().sort((a,b)=>Number(b.id||0)-Number(a.id||0));
  if(status==='ABERTAS') lista=lista.filter(o=>!v20824_osFinal(o));
  else if(status==='FINALIZADAS') lista=lista.filter(o=>v20824_osFinal(o));
  else if(status && status!=='TODAS') lista=lista.filter(o=>norm(o.status||o.statusOs||'ABERTA')===status);
  if(q) lista=lista.filter(o=>norm([o.numero,o.numeroOs,o.id,o.lojaNome,o.prestadorNome,o.status,arr(o.chamados).join(' ')].join(' ')).includes(q));
  const rows=lista.map(o=>{ const ch=os83ChamadosDaOS(d,o); const valor=ch.length?ch.reduce((s,c)=>s+money(c.valor),0):money(o.valorTotal||0); const nums=ch.map(c=>os83Num(c)).filter(Boolean).join(', ')||arr(o.chamados).join(', '); return `<tr><td>${esc(o.numero||o.numeroOs||o.id)}</td><td>${esc(o.lojaNome||'')}</td><td>${esc(o.prestadorNome||'')}</td><td>${esc(nums)}</td><td>${esc(o.status||'ABERTA')}</td><td>${moeda(valor)}</td><td><a class="btn small" href="/os-impressao/${o.id}">🖨️ VER/IMPRIMIR</a> <a class="btn small secondary" href="/os/${o.id}/editar">✏️ EDITAR/JUNÇÃO</a> <form class="inline-form" method="post" action="/os/${o.id}/cancelar"><button class="small danger">CANCELAR</button></form> <form class="inline-form" method="post" action="/os/${o.id}/excluir" onsubmit="return confirm('EXCLUIR ESTA O.S. E LIBERAR CHAMADOS?')"><button class="small danger">🗑️ EXCLUIR</button></form></td></tr>`}).join('');
  res.send(page(req,'ORDENS DE SERVIÇO',`<div class="bar"><h2>📄 ORDENS DE SERVIÇO</h2><a class="btn" href="/os/nova">➕ GERAR O.S.</a></div><form class="card search" method="get" action="/os"><input name="q" value="${esc(req.query.q||'')}" placeholder="BUSCAR Nº O.S., CHAMADO, LOJA OU PRESTADOR"><select name="status"><option ${status==='ABERTAS'?'selected':''}>ABERTAS</option><option ${status==='TODAS'?'selected':''}>TODAS</option><option ${status==='FINALIZADAS'?'selected':''}>FINALIZADAS</option><option ${status==='CANCELADAS'?'selected':''}>CANCELADAS</option></select><button>🔎 BUSCAR</button><a class="btn secondary" href="/os?status=TODAS">MOSTRAR TODAS</a></form><div class="card">${tabela(['Nº','LOJA','PRESTADOR','CHAMADOS','STATUS','VALOR','AÇÕES'],rows,'NENHUMA O.S. ENCONTRADA')}</div>`));
});
...
app.post('/os/gerar',auth,need('ORDENS_SERVICO'),async(req,res)=>{
  ...
  const os={id:next(d,'os'),numero:ref,numeroOs:String(ref),lojaId:ch[0].lojaId||'',prestadorId:ch[0].prestadorId||'',lojaNome:ch[0].lojaNome||ch[0].loja||'',prestadorNome:ch[0].prestadorNome||ch[0].prestador||'',analista:ch[0].analista||'',chamados:ch.map(c=>c.id),valorTotal:ch.reduce((s,c)=>s+money(c.valor),0),status:'ABERTA',observacoes:req.body.observacoes||'',criadoEm:now(),criadoPor:user(req)?.nome||user(req)?.usuario||''};
...
/* PATCH V21.0.1 - PDF DA O.S. PARA DOWNLOAD/COMPARTILHAMENTO */
...
/* PATCH V21.0.1 - editar/remover junção de O.S. */
app.get('/os/:id/editar',auth,need('ORDENS_SERVICO'),(req,res)=>{
  const d=os83State(); const os=(d.os||[]).find(o=>String(o.id)===String(req.params.id)); if(!os)return res.redirect('/os');
  const atuais=os83ChamadosDaOS(d,os); const atualIds=new Set(atuais.map(c=>String(c.id)));
  const baseKey=atuais[0]?os83GroupKey(atuais[0]):`${norm(os.lojaNome)}|||${norm(os.prestadorNome)}`;
  const candidatos=(d.chamados||[]).filter(c=>atualIds.has(String(c.id)) || (!os83Closed(c)&&os83GroupKey(c)===baseKey));
  const rows=candidatos.map(c=>`<tr><td><input type="checkbox" name="chamados" value="${esc(c.id)}" ${atualIds.has(String(c.id))?'checked':''}></td><td>${esc(os83Num(c))}</td><td>${esc(c.lojaNome)}</td><td>${esc(c.prestadorNome)}</td><td>${esc(c.descricao||'')}</td><td>${esc(c.status||'')}</td><td>${moeda(money(c.valor))}</td></tr>`).join('');
  res.send(page(req,'EDITAR O.S.',`<div class="bar"><h2>✏️ EDITAR O.S. ${esc(os.numero||os.id)}</h2><a class="btn secondary" href="/os">VOLTAR</a></div><form class="card form" method="post"><div class="grid4"><label>Status<select name="status"><option ${norm(os.status||'ABERTA')==='ABERTA'?'selected':''}>ABERTA</option><option ${norm(os.status)==='FINALIZADA'?'selected':''}>FINALIZADA</option><option ${norm(os.status)==='CANCELADA'?'selected':''}>CANCELADA</option></select></label><label>Observações<input name="observacoes" value="${esc(os.observacoes||'')}"></label></div><p class="hint">Desmarque o chamado que não fará parte desta junção. Ele será liberado para gerar nova O.S.</p><table><thead><tr><th>USAR</th><th>CHAMADO</th><th>LOJA</th><th>PRESTADOR</th><th>DESCRIÇÃO</th><th>STATUS</th><th>VALOR</th></tr></thead><tbody>${rows}</tbody></table><div class="actions"><button>💾 SALVAR JUNÇÃO</button><a class="btn secondary" href="/os-impressao/${os.id}">IMPRIMIR</a></div></form>`));
});
app.post('/os/:id/editar',auth,need('ORDENS_SERVICO'),(req,res)=>{
  const d=os83State(); const os=(d.os||[]).find(o=>String(o.id)===String(req.params.id)); if(!os)return res.redirect('/os');
  const novos=os83Ids(req.body.chamados); const antigos=os83ChamadosDaOS(d,os);
  antigos.forEach(c=>{ if(!novos.includes(String(c.id))){ delete c.osId; delete c.osNumero; c.statusOs=''; }});
  const ch=(d.chamados||[]).filter(c=>novos.includes(String(c.id)));
  ch.forEach(c=>{c.osId=os.id;c.osNumero=os.numero;c.statusOs='OS GERADA'});
  os.chamados=ch.map(c=>c.id); os.chamadoIds=os.chamados; os.valorTotal=ch.reduce((s,c)=>s+money(c.valor),0); os.status=norm(req.body.status||os.status||'ABERTA'); os.observacoes=req.body.observacoes||''; os.atualizadoEm=now();
  if(os.status==='FINALIZADA') ch.forEach(c=>{c.status='FINALIZADO';c.dataFechamento=c.dataFechamento||now();});
  save(d); res.redirect('/os');
});
app.post('/os/:id/cancelar',auth,need('ORDENS_SERVICO'),(req,res)=>{const d=os83State();const os=(d.os||[]).find(o=>String(o.id)===String(req.params.id));if(os){os.status='CANCELADA';os.canceladoEm=now();}save(d);res.redirect('/os?status=TODAS')});
app.post('/os/:id/excluir',auth,need('ORDENS_SERVICO'),(req,res)=>{const d=os83State();const os=(d.os||[]).find(o=>String(o.id)===String(req.params.id));if(os){os83ChamadosDaOS(d,os).forEach(c=>{delete c.osId;delete c.osNumero;c.statusOs='';});d.os=d.os.filter(o=>String(o.id)!==String(req.params.id));}save(d);res.redirect('/os?status=TODAS')});
...
/* PATCH V21.0.1 - O.S. PDF/WHATSAPP/VALOR */
...
function v20814_whatsLink(tel,msg){ tel=dig(tel); if(!tel)return '#'; if(!tel.startsWith('55'))tel='55'+tel; return `https://wa.me/${tel}`; }
...
function os83Print(req,res,d,os,ch){
  ...
  const showValor=v20814_showValor(req);
  const totalValor=(ch&&ch.length?ch.reduce((sum,c)=>sum+money(c.valor||c.valorServico||0),0):money(os.valorTotal||os.valor||0));
  const desc=ch.map(c=>`${esc(os83Num(c))} ${esc(c.descricao||c.observacoes||'')}${showValor?` - <b>${moeda(money(c.valor||c.valorServico||0))}</b>`:''}`).join('<br>');
...
function v158_atomicSave(d){
  // PATCH V21.0.1: importação precisa salvar pela função oficial save(),
  ...
  try{
    ...
    console.error('V21.0.1 erro save importação:', e.message || e);
  }
...
function v158_safePage(req,title,body){return typeof page==='function'?page(req,title,body):`<!doctype html><html><head><meta charset="utf-8"><link rel="stylesheet" href="/public/style.css"><style>
@media (max-width:768px){
  body{font-size:15px;overflow-x:hidden} header{padding:14px 12px!important} .brand{gap:10px!important;align-items:center!important} .brand img{max-width:76px!important;max-height:54px!important} .brand h1{font-size:24px!important;line-height:1.1!important} .brand p{font-size:13px!important}
  nav,.menu,.nav{display:grid!important;grid-template-columns:1fr 1fr!important;gap:10px!important;width:100%!important} header a.btn,header button,.menu a,.nav a{width:100%!important;min-height:52px!important;padding:14px 10px!important;margin:0!important;font-size:15px!important;border-radius:14px!important;white-space:normal!important;text-align:center!important;display:flex!important;align-items:center!important;justify-content:center!important}
  main{padding:14px!important} .card{padding:14px!important;border-radius:18px!important}.bar{display:flex!important;gap:10px!important;align-items:center!important;flex-wrap:wrap!important}.bar h2{font-size:22px!important;width:100%!important}.bar .btn{flex:1!important;text-align:center!important}
...
/* PATCH V21.0.1 - importação VestCasa sem alterar layout */
...
function v158_buildRows(filePath){
  // PATCH V21.0.1: leitura leve para não derrubar Render/502.
...
function v158_importCore(req, file){
  ...
  v158_atomicSave(d); save(d);
  try{ persistentCache = mergeDB(emptyDB(), d); }catch(e){}
  return {importados,lojasCriadas,prestCriados,ignoradas,duplicados};
}

/* PATCH V21.0.1 - IMPORTAÇÃO ASSÍNCRONA COM LOADING */
  ...
  try{
    ...
    setImmediate(()=>{
      try{
        ...
        console.error('ERRO IMPORTAÇÃO V21.0.1', e);
...
/* PATCH V21.0.2 - PONTO/HORAS COMPLETO COM BUSCA, EDITAR, EXCLUIR E RELATÓRIO */
function phMin(h){ const m=String(h||'').match(/^(\d{1,2}):(\d{2})/); return m?(Number(m[1])*60+Number(m[2])):0; }
function phTotal(inicio,fim){ if(!inicio||!fim)return ''; let a=phMin(inicio),b=phMin(fim); if(b<a)b+=1440; return ((Math.max(0,b-a))/60).toFixed(2)+'h'; }
function phUsuarios(d){
  const mapa=new Map();
  (d.usuarios||[]).forEach(u=>{ if(norm(u.ativo||'SIM')!=='NÃO') mapa.set(norm(u.nome||u.usuario),u.nome||u.usuario); });
  (d.pontos||[]).forEach(p=>{ if(p.usuarioNome) mapa.set(norm(p.usuarioNome),p.usuarioNome); });
  return [...mapa.values()].sort((a,b)=>String(a).localeCompare(String(b),'pt-BR'));
}
function phFiltrar(d,req){
  const usuario=norm(req.query.usuario||req.query.analista||req.query.q||'');
  const de=String(req.query.de||''); const ate=String(req.query.ate||'');
  if(!usuario) return [];
  return (d.pontos||[]).filter(p=>{
    const okUsuario=norm(p.usuarioNome||'').includes(usuario) || String(p.usuarioId||'')===String(req.query.usuarioId||'');
    const okDe=!de || String(p.data||'')>=de;
    const okAte=!ate || String(p.data||'')<=ate;
    return okUsuario && okDe && okAte;
  }).sort((a,b)=>String(b.data||'').localeCompare(String(a.data||''))||String(b.inicio||'').localeCompare(String(a.inicio||'')));
}
function phAssinatura(d,nome){ return v2088_assinaturaHtml(d,nome||''); }

app.get('/ponto-horas',auth,need('PONTO_HORAS'),(req,res)=>{
  const d=load(); d.pontos=d.pontos||[];
  const lista=phFiltrar(d,req);
  const usuarioBusca=req.query.usuario||req.query.analista||req.query.q||'';
  const temBusca=!!norm(usuarioBusca);
  const usuarios=phUsuarios(d);
  const qs=new URLSearchParams({usuario:usuarioBusca||'',de:req.query.de||'',ate:req.query.ate||''}).toString();
  const linhas=temBusca?lista.map(p=>`<tr><td>${esc(p.usuarioNome)}</td><td>${br(p.data)}</td><td>${esc(p.inicio)}</td><td>${esc(p.fim)}</td><td>${esc(p.total||phTotal(p.inicio,p.fim))}</td><td>${esc(p.tipo)}</td><td>${esc(p.obs)}</td><td class="no-print"><a class="btn small" href="/ponto-horas/${p.id}/editar">✏️ Editar</a> <form class="inline-form" method="post" action="/ponto-horas/${p.id}/excluir" onsubmit="return confirm('Excluir este registro de ponto/hora?')"><button class="small danger">🗑️ Excluir</button></form></td></tr>`).join(''):`<tr><td colspan="8" class="muted">Pesquise por usuário/analista para mostrar os pontos batidos.</td></tr>`;
  res.send(page(req,'Ponto/Horas',`<div class="bar"><h2>⏱️ Ponto/Horas</h2><form method="post" action="/ponto-horas/bater"><button>⏱️ Bater ponto agora</button></form></div>
  <form class="card search no-print" method="get"><input name="usuario" placeholder="Pesquisar usuário/analista" value="${esc(usuarioBusca)}" list="ph_users" data-auto="analistas"><datalist id="ph_users">${usuarios.map(u=>`<option value="${esc(u)}"></option>`).join('')}</datalist><input type="date" name="de" value="${esc(req.query.de||'')}"><input type="date" name="ate" value="${esc(req.query.ate||'')}"><button>🔎 Pesquisar</button>${temBusca?`<a class="btn secondary" href="/ponto-horas/relatorio?${qs}">🖨️ Relatório/Imprimir</a>`:''}</form>
  <form class="card search no-print" method="post"><input type="date" name="data" value="${today()}"><input type="time" name="inicio"><input type="time" name="fim"><input name="obs" placeholder="Observação"><button>Salvar hora extra</button></form>
  <div class="card"><table><thead><tr><th>Usuário</th><th>Data</th><th>Início</th><th>Fim</th><th>Total</th><th>Tipo</th><th>Obs</th><th class="no-print">Ações</th></tr></thead><tbody>${linhas}</tbody></table></div>`));
});

app.post('/ponto-horas',auth,need('PONTO_HORAS'),(req,res)=>{
  const d=load(),u=user(req); d.pontos=d.pontos||[];
  d.pontos.push({id:next(d,'ponto'),usuarioId:u.id,usuarioNome:u.nome,data:req.body.data||today(),inicio:req.body.inicio||'',fim:req.body.fim||'',total:phTotal(req.body.inicio,req.body.fim),tipo:'EXTRA',obs:req.body.obs||''});
  save(d); res.redirect('/ponto-horas?usuario='+encodeURIComponent(u.nome));
});

app.post('/ponto-horas/bater',auth,need('PONTO_HORAS'),(req,res)=>{
  const d=load(),u=user(req),dt=new Date(),data=today(),hora=String(dt.getHours()).padStart(2,'0')+':'+String(dt.getMinutes()).padStart(2,'0'); d.pontos=d.pontos||[];
  const aberto=[...d.pontos].reverse().find(p=>String(p.usuarioId)===String(u.id)&&p.data===data&&!p.fim);
  if(aberto){aberto.fim=hora;aberto.total=phTotal(aberto.inicio,aberto.fim);} else d.pontos.push({id:next(d,'ponto'),usuarioId:u.id,usuarioNome:u.nome,data,inicio:hora,fim:'',total:'',tipo:'PONTO',obs:'BATIDA AUTOMÁTICA'});
  save(d); res.redirect('/ponto-horas?usuario='+encodeURIComponent(u.nome));
});

app.get('/ponto-horas/:id/editar',auth,need('PONTO_HORAS'),(req,res)=>{
  const d=load(); const p=(d.pontos||[]).find(x=>String(x.id)===String(req.params.id)); if(!p)return res.redirect('/ponto-horas');
  res.send(page(req,'Editar Ponto/Horas',`<div class="bar"><h2>✏️ Editar Ponto/Horas</h2><a class="btn secondary" href="/ponto-horas?usuario=${encodeURIComponent(p.usuarioNome||'')}">Voltar</a></div><form class="card form" method="post"><div class="grid4"><label>Usuário<input name="usuarioNome" value="${esc(p.usuarioNome||'')}"></label><label>Data<input type="date" name="data" value="${esc(p.data||'')}"></label><label>Início<input type="time" name="inicio" value="${esc(p.inicio||'')}"></label><label>Fim<input type="time" name="fim" value="${esc(p.fim||'')}"></label><label>Tipo<select name="tipo"><option ${norm(p.tipo)==='PONTO'?'selected':''}>PONTO</option><option ${norm(p.tipo)==='EXTRA'?'selected':''}>EXTRA</option></select></label><label>Observação<input name="obs" value="${esc(p.obs||'')}"></label></div><button>💾 Salvar</button></form>`));
});

app.post('/ponto-horas/:id/editar',auth,need('PONTO_HORAS'),(req,res)=>{
  const d=load(); const p=(d.pontos||[]).find(x=>String(x.id)===String(req.params.id)); if(!p)return res.redirect('/ponto-horas');
  Object.assign(p,{usuarioNome:req.body.usuarioNome||p.usuarioNome,data:req.body.data||p.data,inicio:req.body.inicio||'',fim:req.body.fim||'',total:phTotal(req.body.inicio,req.body.fim),tipo:req.body.tipo||p.tipo,obs:req.body.obs||''});
  save(d); res.redirect('/ponto-horas?usuario='+encodeURIComponent(p.usuarioNome||''));
});

app.post('/ponto-horas/:id/excluir',auth,need('PONTO_HORAS'),(req,res)=>{
  const d=load(); const p=(d.pontos||[]).find(x=>String(x.id)===String(req.params.id)); d.pontos=(d.pontos||[]).filter(x=>String(x.id)!==String(req.params.id)); save(d); res.redirect('/ponto-horas?usuario='+encodeURIComponent(p?.usuarioNome||''));
});

app.get('/ponto-horas/relatorio',auth,need('PONTO_HORAS'),(req,res)=>{
  const d=load(); const lista=phFiltrar(d,req); const usuario=req.query.usuario||req.query.analista||req.query.q||''; const logo=appLogo(d); const assinatura=phAssinatura(d,usuario); const periodo=[req.query.de?br(req.query.de):'',req.query.ate?br(req.query.ate):''].filter(Boolean).join(' até ')||'Todos';
  res.send(page(req,'Relatório Ponto/Horas',`<div class="bar no-print"><button onclick="window.print()">🖨️ Imprimir</button><a class="btn secondary" href="/ponto-horas?usuario=${encodeURIComponent(usuario)}&de=${encodeURIComponent(req.query.de||'')}&ate=${encodeURIComponent(req.query.ate||'')}">↩️ Voltar</a></div><section class="card print-report">${logo?`<div class="center"><img style="max-height:70px;max-width:180px" src="${esc(logo)}" onerror="this.style.display='none'"></div>`:''}<h2 class="center">RELATÓRIO DE PONTO/HORAS</h2><p class="center"><b>Usuário/Analista:</b> ${esc(usuario||'')} &nbsp; <b>Período:</b> ${esc(periodo)}</p>${tabela(['Usuário','Data','Início','Fim','Total','Tipo','Obs'],lista.map(p=>`<tr><td>${esc(p.usuarioNome)}</td><td>${br(p.data)}</td><td>${esc(p.inicio)}</td><td>${esc(p.fim)}</td><td>${esc(p.total||phTotal(p.inicio,p.fim))}</td><td>${esc(p.tipo)}</td><td>${esc(p.obs)}</td></tr>`))}<div class="assin-rel"><div>${assinatura||'<div style="height:50px"></div>'}<hr><b>ASSINATURA DIGITAL</b><br>USUÁRIO/ANALISTA</div><div><div style="height:50px"></div><hr><b>ASSINATURA</b></div></div></section><style>.center{text-align:center}.assin-rel{display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:28px;text-align:center}.assin-rel img{max-height:60px;max-width:220px}@media print{header,.no-print,.version{display:none!important}main{padding:0}.card{box-shadow:none;border:0}.print-report{font-size:11px}}</style>`));
});

app.get('/relatorios-ponto',auth,need('PONTO_HORAS'),(req,res)=>res.redirect('/ponto-horas'));
...
let nome=pick(["TITULO DO ESTABELECIMENTO","TÍTULO DO ESTABELECIMENTO","NOME DE FANTASIA","NOME EMPRESARIAL","RAZAO SOCIAL","RAZÃO SOCIAL"]);const rootCnpj=v159_dig(cnpj).slice(0,8);if(rootCnpj==="31035833"||text.includes("MEGA VEST CASA")||text.includes("VESTCASA")||(text.includes("VEST")&&text.includes("CASA")))nome="MEGA VEST CASA";if(!nome)nome=lines.find(l=>/\b(MEGA VEST CASA|VEST CASA|VESTCASA|BURDAYS|BURDAY|LTDA|EIRELI|EPP|ME)\b/.test(l)&&!/ENDERECO|ENDEREÇO|ELETRONICO|ELETRÔNICO|COMPROVANTE|CADASTRO|ATIVIDADE/.test(l))||"";let cidade=pick(["MUNICIPIO","MUNICÍPIO","CIDADE"]);let uf=pick(["UF"]);const fc=v159_find_city(text);if(!cidade||cidade.length<3)cidade=fc.cidade;if(!v159_uf(uf))uf=fc.uf||v159_uf(text);if(!cep&&fc.cep)cep=fc.cep;let endereco=pick(["LOGRADOURO","ENDERECO","ENDEREÇO"]);function afterLogradouro(labels){const start=lines.findIndex(l=>/^LOGRADOURO$|^ENDERECO$|^ENDEREÇO$/.test(l));const end=lines.findIndex((l,i)=>i>start&&/^CEP$/.test(l));const lim=end>start?end:Math.min(lines.length,start+20);for(const lab of labels){for(let i=Math.max(0,start);i<lim;i++){if(lines[i]===lab||lines[i].startsWith(lab+" ")){const same=lines[i].replace(lab,"").replace(/^[:\-\s]+/,"").trim();if(same)return same;for(let k=i+1;k<lim;k++){const v=lines[k];if(v&&!/^(LOGRADOURO|ENDERECO|ENDEREÇO|NUMERO|NÚMERO|COMPLEMENTO|BAIRRO|DISTRITO)$/.test(v))return v;}}}}return "";}let numero=afterLogradouro(["NÚMERO","NUMERO"]);let complemento=afterLogradouro(["COMPLEMENTO"]);return {nome:v159_clean_nome(nome)||"MEGA VEST CASA",cnpj:v159_dig(cnpj),cep:v159_dig(cep),telefone:v159_dig(tel),cidade:v159_clean_cidade(cidade),uf:v159_uf(uf),estado:v159_uf(uf),endereco:v159_up(endereco).replace(/\b(CEP|UF|MUNICIPIO|MUNICÍPIO|ENDERECO ELETRONICO|ENDEREÇO ELETRÔNICO)\b/g," ").replace(/\s+/g," ").trim(),numero:v159_up(numero).replace(/\s+/g," ").trim(),numeroEndereco:v159_up(numero).replace(/\s+/g," ").trim(),complemento:v159_up(complemento).replace(/\s+/g," ").trim()};}
...
/* PATCH V21.0.1 - rotas WhatsApp e PDF da O.S. */
...
/* PATCH V21.0.1 - rotas O.S. PDF e WhatsApp fixas */
app.get('/os-whatsapp-loja/:id',auth,(req,res)=>{
...
/* PATCH V21.0.1 - abrir anexos salvos da loja */
app.get('/lojas/:id/cartao-cnpj', auth, (req,res)=>{
  ...
  if(!data) return res.status(404).send('Cartão CNPJ não encontrado');
  if(String(data).startsWith('data:')){
    const m=String(data).match(/^data:([^;]+);base64,(.*)$/);
    if(m){ res.setHeader('Content-Type',m[1]); return res.end(Buffer.from(m[2],'base64')); }
  }
  res.redirect(data);
});
app.get('/lojas/:id/foto/:idx', auth, (req,res)=>{
  const d=load(); const l=(d.lojas||[]).find(x=>String(x.id)===String(req.params.id));
  const f=(l?.fotos||[])[Number(req.params.idx)]; const data=f?.dataUrl||f?.data;
  if(!data) return res.status(404).send('Foto não encontrada');
  if(String(data).startsWith('data:')){
    const m=String(data).match(/^data:([^;]+);base64,(.*)$/);
    if(m){ res.setHeader('Content-Type',m[1]); return res.end(Buffer.from(m[2],'base64')); }
  }
...
/* PATCH V21.0.1 - STATUS/TESTE DE PERSISTÊNCIA */
app.get('/api/persist/status', auth, (req,res)=>{
  ...
  res.json({
    ...
    totais:{
      ...
      lojas:(d.lojas||[]).length,
      prestadores:(d.prestadores||[]).length,
      chamados:(d.chamados||[]).length,
      os:(d.os||[]).length,
      lembretes:(d.lembretes||[]).length,
      preventivas:(d.preventivas||[]).length
    }
  });
});
app.post('/api/persist/flush', auth, async (req,res)=>{
  const r = await forcePersistNow('flush manual');
  res.json(r);
});


/* PATCH V21.0.1 - status/flush */
app.get('/api/save/status',auth,(req,res)=>{const d=load();res.json({versao:'V21.0.1',supabaseConfigurado:!!supabasePersist,supabaseOk,remoteLoaded,lastSaveOk,lastSaveAt,lastPersistError,totais:{usuarios:(d.usuarios||[]).length,lojas:(d.lojas||[]).length,prestadores:(d.prestadores||[]).length,proprietarios:(d.proprietarios||[]).length,chamados:(d.chamados||[]).length,os:(d.os||[]).length,lembretes:(d.lembretes||[]).length,preventivas:(d.preventivas||[]).length,pontos:(d.pontos||[]).length}})});
app.get('/api/save/flush',auth,async(req,res)=>{try{await saveRemoteNow(load());res.json({ok:true,at:new Date().toISOString()})}catch(e){res.json({ok:false,error:e.message||String(e)})}});
app.post('/api/save/flush',auth,async(req,res)=>{try{await saveRemoteNow(load());res.json({ok:true,at:new Date().toISOString()})}catch(e){res.json({ok:false,error:e.message||String(e)})}});



/* ================= PATCH V20.8.30 - CORREÇÃO GERAL CADASTROS/O.S. + MESCLAGEM SEGURA ================= */
function v20830_limpo(v){
  return String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toUpperCase()
    .replace(/\b(LOJA|FILIAL|UNIDADE|VESTCASA|V\s*&\s*B|VB|BORDADO'S|BORDADOS)\b/g,' ')
    .replace(/[^A-Z0-9]+/g,' ').replace(/\s+/g,' ').trim();
}
function v20830_tipoLoja(nome){
  const n=norm(nome||'');
  if(/\bMEGA\b/.test(n)) return 'MEGA';
  if(/\bSHOPPING\b/.test(n)) return 'SHOPPING';
  if(/\bAG\b|\bAGENCIA\b|\bAG\.\b/.test(n)) return 'AG';
  return 'NORMAL';
}
function v20830_keyLoja(l){
  const nome=l?.nome||l?.nomeLoja||l?.loja||'';
  return v20830_tipoLoja(nome)+'|'+v20830_limpo(nome);
}
function v20830_keyPrest(p){
  const doc=dig(p?.cnpj||p?.cpf||'');
  if(doc.length>=11) return 'DOC|'+doc;
  return 'NOME|'+v20830_limpo(p?.empresa||p?.nome||p?.responsavel||'');
}
function v20830_score(a,b){
  a=v20830_limpo(a); b=v20830_limpo(b); if(!a||!b) return 0;
  if(a===b) return 1;
...
function v20830_mesmoGrupoLoja(a,b){
  const na=a?.nome||a?.nomeLoja||a?.loja||'', nb=b?.nome||b?.nomeLoja||b?.loja||'';
  if(v20830_tipoLoja(na)!==v20830_tipoLoja(nb)) return false;
  return v20830_score(na,nb)>=0.82;
}
function v20830_mergeObj(dest,src){
  for(const [k,v] of Object.entries(src||{})){
    if(['id','analista','analistaResponsavel','responsavelAnalista'].includes(k)) continue;
    if((dest[k]===undefined||dest[k]===null||dest[k]==='') && v!==undefined && v!==null && v!=='') dest[k]=v;
  }
  for(const k of ['fotos','anexos','servicos','tiposServico']){
    const a=arr(dest[k]), b=arr(src?.[k]);
    if(a.length||b.length){
      const seen=new Set(); dest[k]=[...a,...b].filter(x=>{const key=typeof x==='object'?JSON.stringify(x):String(x); if(seen.has(key)) return false; seen.add(key); return true;});
    }
  }
  return dest;
}
function v20830_corrigeReferencias(d,mapLojas,mapPrest){
  (d.chamados||[]).forEach(c=>{
    if(mapLojas[String(c.lojaId)]) c.lojaId=mapLojas[String(c.lojaId)];
    if(mapPrest[String(c.prestadorId)]) c.prestadorId=mapPrest[String(c.prestadorId)];
    const l=(d.lojas||[]).find(x=>String(x.id)===String(c.lojaId)); if(l) c.lojaNome=l.nome||c.lojaNome;
    const p=(d.prestadores||[]).find(x=>String(x.id)===String(c.prestadorId)); if(p) c.prestadorNome=p.empresa||p.nome||p.responsavel||c.prestadorNome;
  });
  (d.os||[]).forEach(o=>{
    if(mapLojas[String(o.lojaId)]) o.lojaId=mapLojas[String(o.lojaId)];
    if(mapPrest[String(o.prestadorId)]) o.prestadorId=mapPrest[String(o.prestadorId)];
    const ch=os83ChamadosDaOS(d,o);
    if(ch[0]){ o.lojaId=ch[0].lojaId||o.lojaId||''; o.prestadorId=ch[0].prestadorId||o.prestadorId||''; o.lojaNome=ch[0].lojaNome||o.lojaNome||''; o.prestadorNome=ch[0].prestadorNome||o.prestadorNome||''; }
    const l=(d.lojas||[]).find(x=>String(x.id)===String(o.lojaId)); if(l) o.lojaNome=l.nome||o.lojaNome;
    const p=(d.prestadores||[]).find(x=>String(x.id)===String(o.prestadorId)); if(p) o.prestadorNome=p.empresa||p.nome||p.responsavel||o.prestadorNome;
  });
}
function v20830_previewMescla(d){
  const lojas=[], prestadores=[];
  const ls=d.lojas||[];
  for(let i=0;i<ls.length;i++) for(let j=i+1;j<ls.length;j++) if(v20830_mesmoGrupoLoja(ls[i],ls[j])) lojas.push({manter:ls[i].id,mesclar:ls[j].id,a:ls[i].nome,b:ls[j].nome,tipo:v20830_tipoLoja(ls[i].nome)});
  const ps=d.prestadores||[];
  for(let i=0;i<ps.length;i++) for(let j=i+1;j<ps.length;j++){
    const ni=ps[i].empresa||ps[i].nome||ps[i].responsavel||'', nj=ps[j].empresa||ps[j].nome||ps[j].responsavel||'';
    if(v20830_keyPrest(ps[i])===v20830_keyPrest(ps[j]) || v20830_score(ni,nj)>=0.88) prestadores.push({manter:ps[i].id,mesclar:ps[j].id,a:ni,b:nj});
  }
  return {lojas,prestadores};
}
async function v20830_aplicaMescla(d){
  const antes={lojas:(d.lojas||[]).length,prestadores:(d.prestadores||[]).length};
  const mapL={}, mapP={};
  const novasL=[];
  (d.lojas||[]).forEach(l=>{
    let dest=novasL.find(x=>v20830_mesmoGrupoLoja(x,l));
    if(!dest){ dest={...l}; novasL.push(dest); }
    else{ v20830_mergeObj(dest,l); dest.analista=''; dest.analistaResponsavel=''; dest.responsavelAnalista=''; mapL[String(l.id)]=dest.id; }
  });
  d.lojas=novasL;
  const novasP=[];
  (d.prestadores||[]).forEach(p=>{
    const nome=p.empresa||p.nome||p.responsavel||'';
    let dest=novasP.find(x=>v20830_keyPrest(x)===v20830_keyPrest(p) || v20830_score(x.empresa||x.nome||x.responsavel||'', nome)>=0.88);
    if(!dest){ dest={...p}; novasP.push(dest); }
    else{ v20830_mergeObj(dest,p); mapP[String(p.id)]=dest.id; }
  });
  d.prestadores=novasP;
  v20830_corrigeReferencias(d,mapL,mapP);
  return {antes,depois:{lojas:d.lojas.length,prestadores:d.prestadores.length},mapL,mapP};
}

/* Reforço O.S. aplicado nas funções principais acima. */


/* PATCH V20.8.30 - ferramenta de manutenção para mesclar duplicados */
app.get('/admin/mesclar-duplicados',auth,need('CONFIG'),(req,res)=>{
  const d=load(); const prev=v20830_previewMescla(d);
  const lojas=prev.lojas.map(x=>`<tr><td>${esc(x.a)}</td><td>${esc(x.b)}</td><td>${esc(x.tipo)}</td></tr>`);
  const prest=prev.prestadores.map(x=>`<tr><td>${esc(x.a)}</td><td>${esc(x.b)}</td></tr>`);
  res.send(page(req,'Mesclar duplicados',`<div class="bar"><h2>🧹 Mesclar duplicados</h2><a class="btn secondary" href="/config">Voltar</a></div><div class="card"><p><b>Seguro:</b> não mescla lojas de tipos diferentes. MEGA, SHOPPING e AG ficam separados quando tiverem essa identificação. Quando mesclar lojas, o analista responsável fica em branco para evitar conflito.</p><form method="post"><button onclick="return confirm('Aplicar mesclagem agora? Faça backup antes se tiver dúvida.')">✅ APLICAR MESCLAGEM</button></form></div><div class="card"><h3>Lojas encontradas: ${prev.lojas.length}</h3>${tabela(['Manter','Mesclar','Tipo'],lojas,'Nenhuma duplicidade provável de loja.')}</div><div class="card"><h3>Prestadores encontrados: ${prev.prestadores.length}</h3>${tabela(['Manter','Mesclar'],prest,'Nenhuma duplicidade provável de prestador.')}</div>`));
});
app.post('/admin/mesclar-duplicados',auth,need('CONFIG'),async(req,res)=>{
  try{ const d=load(); const r=await v20830_aplicaMescla(d); save(d); res.send(page(req,'Mesclagem concluída',`<div class="card"><h2>✅ Mesclagem concluída</h2><p>Lojas: ${r.antes.lojas} → ${r.depois.lojas}</p><p>Prestadores: ${r.antes.prestadores} → ${r.depois.prestadores}</p><a class="btn" href="/admin/mesclar-duplicados">Verificar novamente</a><a class="btn secondary" href="/lojas?mostrar=1">Lojas</a></div>`)); }
  catch(e){ res.status(500).send(errorPage(req,e)); }
});

/* PATCH V20.8.30 - diagnóstico simples das páginas */
app.get('/admin/verificar-paginas',auth,need('CONFIG'),(req,res)=>{
  const rotas=['/','/chamados','/chamados/novo','/lojas','/lojas/novo','/prestadores','/prestadores/novo','/proprietarios','/proprietarios/novo','/lembretes','/preventivas','/os','/os/nova','/ponto-horas','/relatorios','/config'];
  res.send(page(req,'Verificar páginas',`<div class="bar"><h2>🩺 Verificação de páginas</h2><a class="btn secondary" href="/config">Voltar</a></div><div class="card">${rotas.map(r=>`<a class="btn small" href="${r}" target="_blank">Abrir ${r}</a>`).join(' ')}</div>`));
});

app.get(['/api/v2085/status','/api/persist-status'], auth, (req,res)=>{
  const d=load();
  res.json({ok:true,version:'V21.0.1',supabaseConfigurado:!!supabasePersist,supabaseOk,remoteLoaded,lastSaveOk,lastSaveAt,lastRemoteLoadAt,stateId:SUPABASE_STATE_ID,erro:lastPersistError,contagem:{usuarios:(d.usuarios||[]).length,lojas:(d.lojas||[]).length,prestadores:(d.prestadores||[]).length,proprietarios:(d.proprietarios||[]).length,chamados:(d.chamados||[]).length,os:(d.os||[]).length,lembretes:(d.lembretes||[]).length,preventivas:(d.preventivas||[]).length}});
});
...
app.get('/sobre',auth,(req,res)=>{
  const imgPath=path.join(process.cwd(),'public','olitech-sobre.png');
  const img=fs.existsSync(imgPath)?'/public/olitech-sobre.png':'';
  res.send(page(req,'Sobre',`<div class="bar"><h2>ℹ️ Sobre o sistema</h2><a class="btn secondary" href="/">Voltar</a></div><section class="card" style="text-align:center">${img?`<img src="${img}" style="max-width:100%;width:520px;border-radius:16px">`:`<h2>OLITECH</h2><p>Sistema desenvolvido por Olitech - Thiago Lucas de Oliveira em parceria com ChatGPT.</p><p>WhatsApp: 16 96607-6918</p>`}</section>`));
});
...
app.listen(PORT,()=>console.log('V&B Chamados V21.0.1 rodando na porta '+PORT));
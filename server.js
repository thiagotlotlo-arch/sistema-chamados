...
</style></head><body class="theme-${esc(norm(c.tema).toLowerCase())}">${user(req)?`<header><div class="brand">${logo?`<img src="${esc(logo)}" onerror="this.style.display='none'">`:`<div class="logo-fallback">VB</div>`}<div><h1>${esc(c.nomeSistema)}</h1><p>${esc(c.subtitulo)}</p></div></div>${menu(req)}</header>`:''}<main>${body}</main><div class="version">V21.0.3</div><script src="/public/app.js"></script><script>
(function(){
...
app.get('/api/prestadores-sugeridos',auth,(req,res)=>res.json({ok:true,items:sugerePrestadores(load(),req.query.loja||req.query.lojaId,req.query.tipo||req.query.tipoServico)})); app.get('/api/status',auth,(req,res)=>res.json({ok:true,version:'V21.0.3'}));
...
function lojaForm(req,l={}){
  ...
  <form class="card form" method="post" enctype="multipart/form-data">
    ...
    <div class="grid4 loja-grid">
      ...
      <label>Nome loja<input name="nome" value="${esc(l.nome||l.nomeLoja||l.lojaNome||'')}" required></label>

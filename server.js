...
/* PATCH V20.8.17 - função de status fechado para evitar CLOSED IS NOT DEFINED */
...
/* PATCH V20.8.17 - moeda BR correta: R$800,00 = 800.00 */
...
/* PATCH V20.8.17 - consolidação das correções pedidas mantendo estrutura V20.8 */
...
/* PATCH V20.8.17 - LOJA: salvar nome editado, anexos/cartão CNPJ persistentes e reutilizar logo */
...
/* PATCH V20.8.17 - PERSISTÊNCIA SUPABASE APP_STATE SEM ALTERAR FUNÇÕES */
...
async function initPersistentDB(){
  ...
  if(!supabasePersist){
    ...
    console.error('V20.8.17:', lastPersistError);
    return;
  }
  try{
    ...
    if(data && data.data && Object.keys(data.data).length){
      ...
      console.log('V20.8.17 carregado do Supabase app_state:', SUPABASE_STATE_ID);
    }else{
      ...
      if(dbHasRealData(fallback)){
        ...
        console.log('V20.8.17 Supabase estava vazio: enviado backup local com dados para app_state:', SUPABASE_STATE_ID);
      }else{
        const inicial = mergeDB(emptyDB(), {});
        persistentCache = inicial;
        await saveRemoteNow(inicial);
        console.log('V20.8.17 Supabase estava vazio: criado app_state inicial:', SUPABASE_STATE_ID);
      }
      remoteLoaded = true;
    }
  }catch(e){
    supabaseOk = false; remoteLoaded = false; lastSaveOk = false; lastPersistError = e.message || String(e);
    console.error('V20.8.17 ERRO SUPABASE:', lastPersistError);
...
function scheduleRemoteSave(d){
  ...
  try{ fs.writeFileSync(DB_FILE,JSON.stringify(persistentCache,null,2),'utf8'); }catch(e){ console.error('V20.8.17 erro JSON local:', e.message||e); }
  ...
  savingRemote = true;
  setTimeout(async()=>{
    try{ await saveRemoteNow(persistentCache); console.log('V20.8.17 salvo no Supabase app_state:', SUPABASE_STATE_ID); }
    catch(e){ lastSaveOk=false; supabaseOk=false; lastPersistError=e.message||String(e); console.error('V20.8.17 erro ao salvar Supabase:', lastPersistError); }
...
/* PATCH V20.8.17 - assinatura digital do analista na O.S. */
...
function page(req,title,body){const d=load(),c=d.config,logo=appLogo(d);return `<!doctype html><html lang="pt-br"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(c.nomeSistema)} - ${esc(title)}</title><link rel="stylesheet" href="/public/style.css"></head><body class="theme-${esc(norm(c.tema).toLowerCase())}">${user(req)?`<header><div class="brand">${logo?`<img src="${esc(logo)}" onerror="this.style.display='none'">`:`<div class="logo-fallback">VB</div>`}<div><h1>${esc(c.nomeSistema)}</h1><p>${esc(c.subtitulo)}</p></div></div>${menu(req)}</header>`:''}<main>${body}</main><div class="version">V20.8.17</div><script src="/public/app.js"></script>
<script>
(function(){
...
/* PATCH V20.8.17 - HOME MOBILE RÁPIDA: GRID SÓ APÓS BUSCA/MOSTRAR TODOS */
...
app.get('/api/prestadores-sugeridos',auth,(req,res)=>res.json({ok:true,items:sugerePrestadores(load(),req.query.loja||req.query.lojaId,req.query.tipo||req.query.tipoServico)})); app.get('/api/status',auth,(req,res)=>res.json({ok:true,version:'V20.8.17'}));
...
/* PATCH V20.8.17 - preserva nome da loja exatamente como exibido no formulário ao SALVAR */
...
/* PATCH V20.8.17 - nome da loja importada por PDF preservado no salvar */
...
/* ================= PATCH V20.8.17 - OS AGRUPADA + ASSINATURA + WHATSAPP LOJA ================= */
...
/* PATCH V20.8.17 - PDF DA O.S. PARA DOWNLOAD/COMPARTILHAMENTO */
...
/* PATCH V20.8.17 - O.S. PDF/WHATSAPP/VALOR */
...
function v158_atomicSave(d){
  // PATCH V20.8.17: importação precisa salvar pela função oficial save(),
  ...
  try{
    ...
    console.error('V20.8.17 erro save importação:', e.message || e);
  }
...
/* PATCH V20.8.17 - importação VestCasa sem alterar layout */
...
function v158_buildRows(filePath){
  // PATCH V20.8.17: leitura leve para não derrubar Render/502.
...
/* PATCH V20.8.17 - IMPORTAÇÃO ASSÍNCRONA COM LOADING */
  ...
  try{
    ...
    setImmediate(()=>{
      try{
        ...
        console.error('ERRO IMPORTAÇÃO V20.8.17', e);
...
/* PATCH V20.8.17 - rotas WhatsApp e PDF da O.S. */
...
/* PATCH V20.8.17 - rotas O.S. PDF e WhatsApp fixas */
app.get('/os-whatsapp-loja/:id',auth,(req,res)=>{
...
/* PATCH V20.8.17 - abrir anexos salvos da loja */
...
app.listen(PORT,()=>console.log('V&B Chamados V20.8.17 rodando na porta '+PORT));
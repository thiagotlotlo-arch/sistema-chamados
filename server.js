...
function v20820_safeLojaSave(req,l,d){
  ...
  const cidade=String(b.cidade||l.cidade||'').trim().toUpperCase().replace(/\s+/g,' ');
  const ufRaw=String(b.uf||b.estado||l.uf||l.estado||'').trim().toUpperCase().replace(/\s+/g,' ');
  const ufMap={'SÃO PAULO':'SP','SAO PAULO':'SP','CEARÁ':'CE','CEARA':'CE','PARANÁ':'PR','PARANA':'PR','SANTA CATARINA':'SC'};
  const uf=ufMap[ufRaw]||ufRaw;
  const cfg=(d&&d.config)||load().config||{};
  const base=String(cfg.nomeBaseFilial||cfg.nomeBaseLoja||'MEGA VEST CASA').trim().toUpperCase();
  const limpo=nome.replace(/\b(LTDA|ME|EPP|EIRELI|S\/A|SA)\b/g,'').replace(/\s+/g,' ').trim();
  if((limpo===base||limpo==='MEGA VEST CASA')&&cidade&&uf) nome=`${base} ${cidade} ${uf}`;
  if(nome){l.nome=nome;l.nomeLoja=nome;l.lojaNome=nome;}
  ...
  if(!dig(l.cep)&&cidade&&uf){
    const mapa={'PIRACICABA|SP':'13400000','PRAIA GRANDE|SP':'11726000','FORTALEZA|CE':'60000000','SANTOS|SP':'11000000','SAO PAULO|SP':'01001000','SÃO PAULO|SP':'01001000'};
    l.cep=mapa[`${cidade}|${uf}`]||l.cep||'';
  }
  return l;
}
...
(function(){
  ...
  const isNew=/\/(novo|nova|rapido)$/.test(path)||path.includes('/novo');
  if(isEdit && !document.getElementById('btnLiberarEdicao')){
    const form=document.querySelector('form');
    if(form){
      const bar=document.querySelector('.bar,.actions')||form;
      const btn=document.createElement('button');
      btn.type='button'; btn.id='btnLiberarEdicao'; btn.className='btn secondary'; btn.textContent='✏️ EDITAR CAMPOS';
      bar.prepend(btn);
      const fields=[...form.querySelectorAll('input,select,textarea')].filter(x=>x.type!=='hidden'&&x.type!=='submit'&&x.type!=='button'&&x.type!=='file');
      fields.forEach(x=>{x.dataset.wasDisabled=x.disabled?'1':'0'; x.readOnly=true; if(x.tagName==='SELECT') x.disabled=true;});
      btn.onclick=()=>{
        fields.forEach(x=>{x.readOnly=false; if(x.dataset.wasDisabled!=='1') x.disabled=false;});
        btn.textContent='✅ CAMPOS LIBERADOS PARA SALVAR';
      };
      form.addEventListener('submit',()=>{fields.forEach(x=>{if(x.tagName==='SELECT') x.disabled=false; x.readOnly=false;});});
    }
  }
  if(isNew){
    document.querySelectorAll('form input,form select,form textarea').forEach(x=>{x.readOnly=false; if(x.dataset.wasDisabled!=='1') x.disabled=false;});
  }
  document.querySelectorAll('input.money-br,[name="valor"],[name="valorServico"]').forEach(inp=>{
    if(inp.__moneyMask) return; inp.__moneyMask=true;
    inp.addEventListener('blur',()=>{let n=(inp.value||'').replace(/[^\d]/g,''); inp.value=n?(Number(n)/100).toLocaleString('pt-BR',{style:'currency',currency:'BRL'}):'R$ 0,00';});
  });
})();
</script>
...
<script>
(function(){
 if(location.pathname==='/ponto-horas' && !document.getElementById('btnRelHoras')){
  const bar=document.querySelector('.bar,.actions')||document.querySelector('main')||document.body;
  const a=document.createElement('a');a.id='btnRelHoras';a.className='btn';a.textContent='🖨️ RELATÓRIO/IMPRIMIR HORAS';a.href='/ponto-horas/relatorio';
  bar.appendChild(a);
 }
})();
</script>
...
app.get('/ponto-horas/relatorio', auth, need('PONTO_HORAS'), (req,res)=>{
  ...
  let pts=(d.pontos||d.pontoHoras||[]).slice();
  if(q) pts=pts.filter(p=>norm(p.usuario||p.nome||'').includes(norm(q))||String(p.data||'').includes(q));
  const rows=pts.map(p=>`<tr><td>${esc(p.usuario||p.nome||'')}</td><td>${esc(p.data||'')}</td><td>${esc(p.inicio||p.entrada||'')}</td><td>${esc(p.fim||p.saida||'')}</td><td>${esc(p.total||'')}</td><td>${esc(p.tipo||'')}</td><td>${esc(p.obs||p.observacao||'')}</td></tr>`).join('');
  res.send(page(req,'Relatório de horas',`<div class="bar no-print"><h2>🕐 RELATÓRIO DE HORAS</h2><button onclick="window.print()">🖨️ IMPRIMIR</button><a class="btn secondary" href="/ponto-horas">VOLTAR</a></div>
  <form class="card no-print" method="get"><input name="q" placeholder="USUÁRIO OU DATA" value="${esc(q)}"><button>BUSCAR</button></form>
  <div class="card"><h2>RELATÓRIO DE HORAS</h2><table><thead><tr><th>USUÁRIO</th><th>DATA</th><th>INÍCIO</th><th>FIM</th><th>TOTAL</th><th>TIPO</th><th>OBS</th></tr></thead><tbody>${rows||'<tr><td colspan="7">NENHUM REGISTRO</td></tr>'}</tbody></table></div>
  <style>@media print{.no-print,header,nav,.menu{display:none!important}.card{box-shadow:none!important;border:0!important}body{background:white!important}}</style>`));
});
